				...
				GenomeMouseOn.on("mouseover",function(d){console.log(d);
					GenomeMouseOnTooltip.html(self.settings.GenomeMouseOverTooltipsHtml01 + strip(d.value))
                           ...
                           d3.select(this)
                              ...
                              .style("opacity",  function(d,i) { if(self.settings.GenomeMouseOverOpacity=="none"){return "";}else{return self.settings.GenomeMouseOverOpacity;} })
                              .style("stroke", function(d,i) { if(self.settings.GenomeMouseOverStrokeColor=="none"){return "";}else{return self.settings.GenomeMouseOverStrokeColor;} })
                              .style("stroke-width", function(d,i) { if(self.settings.GenomeMouseOverStrokeWidth=="none"){return "";}else{return self.settings.GenomeMouseOverStrokeWidth;} });
		    ...
		    if(self.settings.GenomeMouseOutDisplay==true){
                   GenomeMouseOn.on("mouseout",function(d){
                       ...
                       d3.select(this)
                          ...
                          .style("fill",  function(d,i) { if(self.settings.GenomeMouseOutColor=="none"){return fill(d.index);}else{return self.settings.GenomeMouseOutColor;} })
							...
							if(self.HISTOGRAM.length > 0){
								...
								for(var histogrami=0; histogrami<self.HISTOGRAM.length; histogrami++){
									...
									svg2.append("g")
										...
										.attr("class", "BioCircosHISTOGRAM")
										.attr("fill", self.HISTOGRAMsettings.histogramFillColor)
										.attr("d", d3.svg.arc().innerRadius(self.HISTOGRAMsettings.minRadius).outerRadius(function(d) {return self.HISTOGRAMsettings.minRadius + ((d.histogram_value-self.histogram_value_maxmin(ChromHist[histogrami])[1])*(self.HISTOGRAMsettings.maxRadius-self.HISTOGRAMsettings.minRadius)/(self.histogram_value_maxmin(ChromHist[histogrami])[0]-self.histogram_value_maxmin(ChromHist[histogrami])[1]));}));
							...
							if(self.LINE.length > 0){
								...
								for(var linei=0; linei<self.LINE.length; linei++){
										...
										console.log(line_objects);
											...
											var line_objects_a_chr = line_objects.filter(function(element,pos){return element.line_chr==self.genomeLabel[d.index]})
