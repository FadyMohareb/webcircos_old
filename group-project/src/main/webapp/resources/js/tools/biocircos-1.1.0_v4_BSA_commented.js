...
(function ($) {
	//this BioCIrcos function initialises the whole BioCircos adding data to be used by the oither functions, which are prototypes set to the BIoCircos function
    BioCircos = function () {
        var self = this;
		//if number of arguments passed to biocircos is more than 2
        if (arguments.length >= 2) {
			...
			//searches for the regular expression, and if it finds it adds the data argument to a self contained array
            self.CNV = new Array();
...
//            //SIngle CHromosome Bins
//            self.sHEATMAP = new Array();
//            self.sHEATMAPConfig = new Array();
//            for (var n = 0; n < arguments.length; n++) {
//                var reg = /HeChrom$/;
//                if (reg.test(arguments[n][0])) {
//                    self.sHEATMAPConfig.push(arguments[n][1]);
//                    self.sHEATMAP.push(arguments[n][2]);
//                }
//            }
            
//            self.sHEATMAP2 = new Array();
//            self.sHEATMAP2Config = new Array();
//            for (var n = 0; n < arguments.length; n++) {
//                var reg = /02HeChrom$/;
//                if (reg.test(arguments[n][0])) {
//                    self.sHEATMAP2Config.push(arguments[n][1]);
//                    self.sHEATMAP2.push(arguments[n][2]);
//                }
//            }
			//searches for the regular expression, and if it finds it adds the data argument to a self contained array
...
//            self.sHISTOGRAM = new Array();
//            self.sHISTOGRAMConfig = new Array();
//            for (var n = 0; n < arguments.length; n++) {
//                var reg = /HChrom$/;
//                if (reg.test(arguments[n][0])) {
//                    self.sHISTOGRAMConfig.push(arguments[n][1]);
//                    self.sHISTOGRAM.push(arguments[n][2]);
//                }
//            }
		...
		//SELF.SETTINGS is a json array that presets default values for all the properties in  the BioCircos library
        self.settings = {
            "target": "biocircos",
            "svgWidth": 900,
            "svgHeight": 600,
            ...
            "outerRadius": 270,
            "zoom": false,
            "genomeFillColor": ["rgb(0,255,0)", "rgb(255,255,0)", "rgb(255,0,255)", "rgb(204,0,0)", "rgb(255,0,0)", "rgb(255,0,204)", "rgb(255,204,204)", "rgb(255,153,0)", "rgb(255,204,0)", "rgb(255,255,0)", "rgb(204,255,0)", "rgb(0,255,0)", "rgb(53,128,0)", "rgb(0,0,204)", "rgb(102,153,255)", "rgb(153,204,255)", "rgb(0,255,255)", "rgb(204,255,255)", "rgb(153,0,204)", "rgb(204,51,255)", "rgb(204,153,255)", "rgb(102,102,102)", "rgb(153,153,153)", "rgb(204,204,204)"],
            ...
            "BSADividers": true,
            "BSABackground": false,
            "BSALINKMouseEvent": true,
            "BSALINKMouseOverDisplay": true,
            "BSALINKMouseOutDisplay": true,
            "BSAGenomeMouseEvent": true,
            "BSAGenomeMouseOverDisplay": true,
            "BSAGenomeMouseOutDisplay": true,

            "svgDrawn": false,
            "SingleSvgDrawn": false,
            ...
            "HISTOGRAMMouseOverTooltipsHtml06": "<br>SNP Count: ",
		...
		//these settings are a json array of the default values for the various modules in BioCircos
        self.CNVsettings = {
            "maxRadius": 200,
            "minRadius": 190,
            "CNVwidth": 10,
        ...
        //these functions, defined below, are taken from the self.settings onject, or the BioCircossettings defined when the callback is initialised on a page
		//self.update_settings is a function that updates the settings from the self.settings object earlier using jquery, replaces them with the settings defined from the page
		self.update_settings(self.argumentsBiocircosSettings);
        self.BSABackground = self.settings.BSABackground;
        self.BSABorder = self.settings.BSABorder;
        self.BSADividers = self.settings.BSADividers;
        self.BSATextDisplay = self.settings.BSATextDisplay;
        self.BSAzoom = self.settings.BSAzoom;
        self.textTrack = self.settings.textTrack;
        self.singleSvgDrawn = self.settings.SingleSvgDrawn;
        self.svgDrawn = self.settings.svgDrawn;
		...
		//init genome is a object that creates key value object for all teh chromosome names, setting the value as corresponding integer, so the chromosomes can be converted into numbers later in the function
	...
	//update settings functions use jquery to update the settings in self.settings with the settings defined in the arguments passed to BioCircos, one for each module and BioCircos Settings
    ...
    BioCircos.prototype.update_ARCsettings = function (settings_object) {
		...
		//honestly not sure what these do, seem to be the same settings as above in the self.settings section, but as functions
	...
	//this genome matrix function takes the biocircos genome object argument, and creates a genomeLabel array with all the chromosome names, and 
	// a genome matrix called genome length, which is an array of arrays, where the first element of the arrays is the length of that chromosome, and the 
	//rest is the number of other chromosomes, but length set at 0 eg. [[20000,0,0,0,0,0,0,][2569038,0,0,0,0,0,0]], this is the correct format for d3.layout.chord
	...
	//gets the maximum and minimum value for BioCircos heatmap array. 
	//this is the function that crashed during the single chromosome bin fiasco
	//the max and min value are used to determine the interpolator value changing the colour in the heatmap
    BioCircos.prototype.heatmap_value_maxmin = function (heatmapIn) {
        var self = this;
        var i = heatmapIn.length;
        var heatmapValueList = new Array();
        for (var k = 0; k < i; k++) {
            heatmapValueList.push(heatmapIn[k].value);
        }
	...
	//not using this functionm but gets the same max and min for the snp values, to place the snps somewhere in the middle i.e
	//sets the maximum at the outerradius of the arc, and mimum at inner radius then calculates the location of the other snps as somehwere in the middle
	...
	//same for CNV as above, we're not using this
	...
	//this is where the magic happens, only functions used by webcircos are commented
    BioCircos.prototype.draw_genome = function (genome) {
        //declare var self as global between all functions with this declaration
		var self = this;
		//generate a chord set for the genome matrix generated in the function .genome_matrix
		//using d3.layout.chord. the chord.groups can be passed to d3.svg.arc to draw an arc for the genome
        var chord = d3.layout.chord()
                .padding(self.chrPad)
                .sortSubgroups(d3.descending)
                .matrix(genome);
		//set the svg width and height, as well as the arc inner and outer radius
        var width = self.svgWidth,
                height = self.svgHeight,
                innerRadius = self.innerRadius,
                outerRadius = self.outerRadius;
		//create the fill array using an ordinal scale
        var fill = d3.scale.ordinal()
                .domain(d3.range(4))
                .range(self.genomeFillColor);
		//sum the genome, to be used in text tracks,a s they must span the whole width of the genome
        var GenomeSum = function(genome){
			var arr = [];
			for(var n = 0; n < genome.length; n++){
				arr.push(genome[n][0])
			}
			return arr;
		}
		
		var sum = GenomeSum(genome).reduce(add, 0);
		function add(a, b) {
				return a + b;
		};
		console.log(sum);
		var ArrayCeption = new Array();
		ArrayCeption.push(sum);
		
		
		var SingleArray = new Array();
		SingleArray.push(ArrayCeption);
		
		var TextChord = d3.layout.chord()
			.padding(0)
			.sortSubgroups(d3.descending)
			.matrix(SingleArray);
		...
		//if self.zoom == true append the svg to the specified target div, add the zoom funcrion and translate the svg to fit the new zoom ratio
		//the svg has the width and height set above
        if (self.zoom == true) {
		...
		// else svg with no zoom function
        } else {
            var svg = d3.select(self.target).append("svg")
                    .attr("width", width)
                    .attr("height", height)
		...
		//if self.genome border display == true append a g element to the SVG
		//append all the chromosome arcs, 
        if (self.genomeBorderDisplay == true) {
            svg.append("g")
				.attr("class", "BioCircosGenome")
				.attr("id", "BioCircosGenome")
				.selectAll("path.BioCircosGenome")
				.data(chord.groups)
				.enter().append("path")
				.attr("class", "BioCircosGenome")
				.attr("id",function(d,i){
				return "BioCircosGenome_" +i; //give unique id to each arc
				})
				...
				.each(function(d,i){ //append invisible arcs to outer edge of each chromosome arc, for drawing names on 
					var firstArcSection = /(^.+?)L/;
					var newArc = firstArcSection.exec(d3.select(this).attr("d"))[1];
					newArc = newArc.replace(/,/g ," ");
					//flip upside names the rigth way up
					if(d.endAngle > 90 * Math.PI/180 && d.endAngle < 280 * Math.PI/180){
						//regex to create new arcs
						var startLoc = /M(.*?)A/,
						 middleLoc = /A(.*?)0 0 1/,
						 endLoc = /0 0 1 (.*?)$/;
						...
						//append the invisible arcs
						svg.append("path")
								.attr("class", "hiddenArcs")
								.attr("id", "BioCircosGenomes_" + i)
								.attr("d", newArc)
								.style("fill", "none");
						
				});
			
        }else{	//otherwise boring old arcs, no borders
			svg.append("g")
				.attr("class", "BioCircosGenome")
				.selectAll("path.BioCircosGenome")
				.data(chord.groups)
        ...
        if (self.settings.GenomeMouseEvent == true) {
			//append a div to the body that contains will contain the tooltip
            var GenomeMouseOnTooltip = d3.select("body")
				.append("div")
				.attr("class", "BioCircosGenomeTooltip")
				.attr("id", "BioCircosGenomeTooltip")
				.style("opacity", 0);
			//function for the mouse on, select all paths with class of BioCircosGenome
            var GenomeMouseOn = svg.selectAll("path.BioCircosGenome");

			
            if (self.settings.GenomeMouseOverDisplay == true) {
                //strip is a function to remove floating point precision errors
				var strip = function (value, precision) {
                    var power = Math.pow(10, precision || 0);
                    return String(Math.round(value * power) / power);
                };
				//on mouseover, perform the following functions
                GenomeMouseOn.on("mouseover", function (d) {
                    console.log(d);
					//create a tooltip, use jquery .html to set the value to the stipped value of that chromosome, which is equivalent to the length
                    GenomeMouseOnTooltip.html(self.settings.GenomeMouseOverTooltipsHtml01 + strip(d.value))
                    ...
                    //set the styles for this mouseover
					d3.select(this)
						.style("fill", function (d, i) { //chromosome highlight colour
							if (self.settings.GenomeMouseOverColor == "none") {
								return "";
							} else {
								return self.settings.GenomeMouseOverColor;
							}
						})
						.style("opacity", function (d, i) { //opacity
							if (self.settings.GenomeMouseOverOpacity == "none") {
								return "";
							} else {
								return self.settings.GenomeMouseOverOpacity;
							}
						})
						.style("stroke", function (d, i) { //highlight border colour
							if (self.settings.GenomeMouseOverStrokeColor == "none") {
								return "";
							} else {
								return self.settings.GenomeMouseOverStrokeColor;
							}
						})
						.style("stroke-width", function (d, i) { //mouse over border width
							if (self.settings.GenomeMouseOverStrokeWidth == "none") {
								return "";
							} else {
								return self.settings.GenomeMouseOverStrokeWidth;
							}
						});

                })
            }
			//when the mouse leaves the chromosome, set the every display in teh chromsome back to how it was o
                    ...
                    d3.select(this)
                            ...
                            .style("opacity", function (d, i) {
                                if (self.settings.GenomeMouseOutOpacity == "none") {
                                    ...
                                    return self.settings.GenomeMouseOutOpacity;
                                }
                            })
                            .style("stroke", function (d, i) {
                                if (self.settings.GenomeMouseOutStrokeColor == "none") {
                                    return "";
                                } else {
                                    return self.settings.GenomeMouseOutStrokeColor;
                                }
                            })
                            .style("stroke-width", function (d, i) {
                                if (self.settings.GenomeMouseOutStrokeWidth == "none") {
                                    return "";
                                } else {
                                    return self.settings.GenomeMouseOutStrokeWidth;
                                }
                            });
                });
            }
			//create the click function, that switches from cgenome to single chromsome view
            if (self.settings.GenomeSingleView == true) {


                GenomeMouseOn.on("click", function (d) {
                    
					var active = BigBioCircos.active ? false : true,
                            newOpacity = active ? 0 : 1;
					//sets the state of big biocircos active to false, and changes the opacity to 0
                    d3.select("#BigBioCircos").style("opacity", newOpacity);
                    //hides the genome tooltip
					d3.select("div.BioCircosGenomeTooltip").style("opacity", 0.0);
                    //calls the draw single chromosome view function
					self.chromview(genome, d);
                    BigBioCircos.active = active;
                });
                    console.log(BigBioCircos.active + " BioCircosACtive");
            }
            
        }
        if (self.genomeTextDisplay == true) {            
			//appends the text labels to the svg
            svg.append("g").selectAll("text")
                    .data(chord.groups) // for chord data
                    .enter().append("text")
                    .style("fill", self.genomeTextColor)
                    .style("font-size", self.genomeTextSize)
                    .attr("class", "donutText")
                    .attr("dy", function(d,i){ //if the angle is at the bottom half of the arc, set dy to move it down rather than up
                    if(d.endAngle > 90*Math.PI/180 && d.endAngle < 280 * Math.PI/180){
                        return 25;
                    }else{
                        return -25;
                   ...
                   .attr("xlink:href", function(d,i){return "#BioCircosGenomes_"+i})
                    .text(function (d, i) {
                        return self.genomeLabel[i];})
                    .style("font-size", "8");};        
        if (self.ticksDisplay == true) {
            //append ticks to the svg
			var ticks = svg.append("g").selectAll("g")
                    ...
                    .attr("transform", function (d) {
                        return "rotate(" + (d.angle * 180 / Math.PI - 90) + ")" + "translate(" + (outerRadius - 0) + ",0)";
                    });
		...
		//mow we get on to the modules and how they are drawn
		//I'm only commented the modules we used
		//if the regex matched an argument and then added that array, we then do it for all the elements in this array
        if (self.BACKGROUND.length > 0) {
			//for all the elements in this array
            for (var backgroundi = 0; backgroundi < self.BACKGROUND.length; backgroundi++) {
                //update settings with passed argument array
				self.update_BACKGROUNDsettings(self.BACKGROUNDConfig[backgroundi]);
				//append path to svg, to draw using chord.groups data, and the configuration data in the Background object
				...
				//append axes using chord.groups data, and the configuration data in the Background object
		...
		//if theres an arc module
        if (self.ARC.length > 0) {
            //for all of that arc module arrays passed
            for (var arci = 0; arci < self.ARC.length; arci++) {
				//update arc settings with the ones found in the objects
                self.update_ARCsettings(self.ARCConfig[arci]);
                //failed text label attempt
				if(self.textTrack == true){
                svg.append("g")		
                .attr("class","TextTrackS")
                .attr("id","TextTrackS")
                .selectAll("path.TextTrack")
                .data(TextChord.groups)
                .enter().append("path")
                .attr("class","TextTrack")
                .attr("id", "TextTrack")            
                .attr("d", d3.svg.arc().innerRadius((innerRadius -10)).outerRadius((innerRadius-10)))
                .style("fill", "none")
                .style("stroke", "none")
                ...
                svg.append("text")
				.attr("id", "BioCircosARCText")
				.attr("dy", +0).attr("text-anchor", "start")
				.attr("font-size", 8)
				.append("textPath")
                                
				.attr("xlink:href", "#TextTrack")
				.text("Annotation")
                .attr("x", 0);
                
                }
                //create the arc_objects object value array calling BioCIrcos Arc function
				var arc_objects = BioCircosArc(chord.groups());
				//function that draws the arc of set inner and outer radius
                var arc = d3.svg.arc()
                        .innerRadius(innerRadius + self.ARCsettings.innerRadius)
                        .outerRadius(outerRadius + self.ARCsettings.outerRadius);
				
				//this code appends a g element and for each arc data object draws a tiny arc to represent it
                svg.append("g")
				...
				//this function takes the arc array, and passes it to D3.map which splits the array entry, and allows you to return a mixture of chord.groupadata and arc object data
				//this can be used to draw an arc in the right place for each chromosome, and the properties for that tiny arc
				//d[self.initGenome[v.chr]] is the chromsome number
                function BioCircosArc(d) {
                    return self.ARC[arci].map(function (v, i) {
                        ...
                        console.log(arc_k); //arc_k is the radians that one base is worth, so you can figure out how wide a gene N bases long is
                        return {
                            arck: arc_k,
			...
			//the tooltips all work the same, so they append a div, on mouse over they display tooltips using jquery.html function
			//the settings are set below, and on mouse out it returns to normal, and dissapears
			//we only use the ARCMouseOverDisplay and ARCMouseOutDisplay functions
            if (self.settings.ARCMouseEvent == true) {
                ...
                if (self.settings.ARCMouseClickDisplay == true) {
                    ARCMouseOn.on("click", function (d) {
                        d3.select(this)
                                .style("fill", function (d, i) {
                                    if (self.settings.ARCMouseClickColor == "none") {
                                        return "";
                                    } else {
                                        return self.settings.ARCMouseClickColor;
                                    }
                                })
                                .style("opacity", function (d, i) {
                                    if (self.settings.ARCMouseClickArcOpacity == "none") {
                                        return "";
                                    } else {
                                        return self.settings.ARCMouseClickArcOpacity;
                                    }
                                })
                                .style("stroke", function (d, i) {
                                    if (self.settings.ARCMouseClickArcStrokeColor == "none") {
                                        return "";
                                    } else {
                                        return self.settings.ARCMouseClickArcStrokeColor;
                                    }
                                })
                                .style("stroke-width", function (d, i) {
                                    if (self.settings.ARCMouseClickArcStrokeWidth == "none") {
                                        return "";
                                    } else {
                                        return self.settings.ARCMouseClickArcStrokeWidth;
                                    }
                                });
                ...
                if (self.settings.ARCMouseDownDisplay == true) {
                    ARCMouseOn.on("mousedown", function (d) {
                        d3.select(this)
                                .style("fill", function (d, i) {
                                    if (self.settings.ARCMouseDownColor == "none") {
                                        return "";
                                    } else {
                                        return self.settings.ARCMouseDownColor;
                                    }
                                })
                                .style("opacity", function (d, i) {
                                    if (self.settings.ARCMouseDownArcOpacity == "none") {
                                        return "";
                                    } else {
                                        return self.settings.ARCMouseDownArcOpacity;
                                    }
                                })
                                .style("stroke", function (d, i) {
                                    if (self.settings.ARCMouseDownArcStrokeColor == "none") {
                                        return "";
                                    } else {
                                        return self.settings.ARCMouseDownArcStrokeColor;
                                    }
                                })
                                .style("stroke-width", function (d, i) {
                                    if (self.settings.ARCMouseDownArcStrokeWidth == "none") {
                                        return "";
                                    } else {
                                        return self.settings.ARCMouseDownArcStrokeWidth;
                                    }
                                });
                    })
                }
                ...
                if (self.settings.ARCMouseEnterDisplay == true) {
                    ARCMouseOn.on("mouseenter", function (d) {
                        d3.select(this)
                                .style("fill", function (d, i) {
                                    if (self.settings.ARCMouseEnterColor == "none") {
                                        return "";
                                    } else {
                                        return self.settings.ARCMouseEnterColor;
                                    }
                                })
                                .style("opacity", function (d, i) {
                                    if (self.settings.ARCMouseEnterArcOpacity == "none") {
                                        return "";
                                    } else {
                                        return self.settings.ARCMouseEnterArcOpacity;
                                    }
                                })
                                .style("stroke", function (d, i) {
                                    if (self.settings.ARCMouseEnterArcStrokeColor == "none") {
                                        return "";
                                    } else {
                                        return self.settings.ARCMouseEnterArcStrokeColor;
                                    }
                                })
                                .style("stroke-width", function (d, i) {
                                    if (self.settings.ARCMouseEnterArcStrokeWidth == "none") {
                                        return "";
                                    } else {
                                        return self.settings.ARCMouseEnterArcStrokeWidth;
                                    }
                                });
                    })
                }
                ...
                if (self.settings.ARCMouseLeaveDisplay == true) {
                    ARCMouseOn.on("mouseleave", function (d) {
                        ...
                        d3.select(this)
                                .style("fill", function (d, i) {
                                    if (self.settings.ARCMouseLeaveColor == "none") {
                                        return "";
                                    } else {
                                        return self.settings.ARCMouseLeaveColor;
                                    }
                                })
                                .style("opacity", function (d, i) {
                                    if (self.settings.ARCMouseLeaveArcOpacity == "none") {
                                        return "";
                                    } else {
                                        return self.settings.ARCMouseLeaveArcOpacity;
                                    }
                                })
                                .style("stroke", function (d, i) {
                                    if (self.settings.ARCMouseLeaveArcStrokeColor == "none") {
                                        return "";
                                    } else {
                                        return self.settings.ARCMouseLeaveArcStrokeColor;
                                    }
                                })
                                .style("stroke-width", function (d, i) {
                                    if (self.settings.ARCMouseLeaveArcStrokeWidth == "none") {
                                        return "";
                                    } else {
                                        return self.settings.ARCMouseLeaveArcStrokeWidth;
                                    }
                                });
                    })
                }
                ...
                if (self.settings.ARCMouseUpDisplay == true) {
                    ARCMouseOn.on("mouseup", function (d) {
                        d3.select(this)
                                .style("fill", function (d, i) {
                                    if (self.settings.ARCMouseUpColor == "none") {
                                        return "";
                                    } else {
                                        return self.settings.ARCMouseUpColor;
                                    }
                                })
                                .style("opacity", function (d, i) {
                                    if (self.settings.ARCMouseUpArcOpacity == "none") {
                                        return "";
                                    } else {
                                        return self.settings.ARCMouseUpArcOpacity;
                                    }
                                })
                                .style("stroke", function (d, i) {
                                    if (self.settings.ARCMouseUpArcStrokeColor == "none") {
                                        return "";
                                    } else {
                                        return self.settings.ARCMouseUpArcStrokeColor;
                                    }
                                })
                                .style("stroke-width", function (d, i) {
                                    if (self.settings.ARCMouseUpArcStrokeWidth == "none") {
                                        return "";
                                    } else {
                                        return self.settings.ARCMouseUpArcStrokeWidth;
                                    }
                                });
                    })
                }
                ...
                if (self.settings.ARCMouseMoveDisplay == true) {
                    ARCMouseOn.on("mousemove", function (d) {
                        d3.select(this)
                                .style("fill", function (d, i) {
                                    if (self.settings.ARCMouseMoveColor == "none") {
                                        return "";
                                    } else {
                                        return self.settings.ARCMouseMoveColor;
                                    }
                                })
                                .style("opacity", function (d, i) {
                                    if (self.settings.ARCMouseMoveArcOpacity == "none") {
                                        return "";
                                    } else {
                                        return self.settings.ARCMouseMoveArcOpacity;
                                    }
                                })
                                .style("stroke", function (d, i) {
                                    if (self.settings.ARCMouseMoveArcStrokeColor == "none") {
                                        return "";
                                    } else {
                                        return self.settings.ARCMouseMoveArcStrokeColor;
                                    }
                                })
                                .style("stroke-width", function (d, i) {
                                    if (self.settings.ARCMouseMoveArcStrokeWidth == "none") {
                                        return "";
                                    } else {
                                        return self.settings.ARCMouseMoveArcStrokeWidth;
                                    }
                                });
		...
		//if there is a histogram object		
        if (self.HISTOGRAM.length > 0) {
            ...
            for (var histogrami = 0; histogrami < self.HISTOGRAM.length; histogrami++) {
				...
				//create histogram objects, same way as arc objects contain all necessary data to draw the histogram
                var histogram_objects = BioCircosHISTOGRAM(chord.groups())
				//failed text track attempt
			   if(self.textTrack == true){
                    	svg.append("g")		
                        .attr("class","TextTrackS2")
                        .attr("id","TextTrackS2")
                        .selectAll("path.TextTrack2")
                        .data(TextChord.groups)
                  .enter().append("path")
                        .attr("class","TextTrack2")
                        .attr("id", "TextTrack2")            
                        .attr("d", d3.svg.arc().innerRadius((0+self.HISTOGRAMsettings.minRadius)).outerRadius((0 + self.HISTOGRAMsettings.maxRadius)))
                        .style("fill", "none")
                        .style("stroke", "none")
				...
				.attr("font-size", 8)
				.append("textPath")
				.attr("xlink:href", "#TextTrack2")
				.text("SNP density");
                    
                }
				//append a g element, draw the histogram from it, 
				svg.append("g")
					.attr("class", "BioCircosHISTOGRAMMain")
					...
					.attr("d", d3.svg.arc().innerRadius(self.HISTOGRAMsettings.minRadius).outerRadius(function (d) { //the outer radius is calculated using maximimum value - the differnec in values
						return self.HISTOGRAMsettings.minRadius + ((d.histogram_value - self.histogram_value_maxmin(self.HISTOGRAM[histogrami])[1]) * (self.HISTOGRAMsettings.maxRadius - self.HISTOGRAMsettings.minRadius) / (self.histogram_value_maxmin(self.HISTOGRAM[histogrami])[0] - self.histogram_value_maxmin(self.HISTOGRAM[histogrami])[1]));
					}));
                self.init_HISTOGRAMsettings();
				//BioCircosHISTOGRAM creates the histogram objects by for each chord data passing the BioCircos Histogram data points to a map, and creating key value object for each entry 
				//in the histogram array and returning them to D3.svg.arc to draw and then sets the properties accordingly
                function BioCircosHISTOGRAM(d) {
                    return self.HISTOGRAM[histogrami].map(function (v, i) { //d[self.initGenome[v.chr]] is the chromsome number
            ...
            if (self.settings.HISTOGRAMMouseEvent == true) {
                ...
                if (self.settings.HISTOGRAMMouseOverDisplay == true) {
                    HISTOGRAMMouseOn.on("mouseover", function (d) {
                        ...
                        d3.select(this)
                                .style("fill", function (d, i) {
                                    if (self.settings.HISTOGRAMMouseOverColor == "none") {
                                        return "";
                                    } else {
                                        return self.settings.HISTOGRAMMouseOverColor;
                                    }
                                })
                                .style("opacity", function (d, i) {
                                    if (self.settings.HISTOGRAMMouseOverOpacity == "none") {
                                        return "";
                                    } else {
                                        return self.settings.HISTOGRAMMouseOverOpacity;
                                    }
                                })
                                .style("stroke", function (d, i) {
                                    if (self.settings.HISTOGRAMMouseOverStrokeColor == "none") {
                                        return "";
                                    } else {
                                        return self.settings.HISTOGRAMMouseOverStrokeColor;
                                    }
                                })
                                .style("stroke-width", function (d, i) {
                                    if (self.settings.HISTOGRAMMouseOverStrokeWidth == "none") {
                                        return "";
                                    } else {
                                        return self.settings.HISTOGRAMMouseOverStrokeWidth;
                                    }
                                });
                    })
                }
                if (self.settings.HISTOGRAMMouseClickDisplay == true) {
                    HISTOGRAMMouseOn.on("click", function (d) {
                        d3.select(this)
                                .style("fill", function (d, i) {
                                    if (self.settings.HISTOGRAMMouseClickColor == "none") {
                                        return "";
                                    } else {
                                        return self.settings.HISTOGRAMMouseClickColor;
                                    }
                                })
                                .style("opacity", function (d, i) {
                                    if (self.settings.HISTOGRAMMouseClickOpacity == "none") {
                                        return "";
                                    } else {
                                        return self.settings.HISTOGRAMMouseClickOpacity;
                                    }
                                })
                                .style("stroke", function (d, i) {
                                    if (self.settings.HISTOGRAMMouseClickStrokeColor == "none") {
                                        return "";
                                    } else {
                                        return self.settings.HISTOGRAMMouseClickStrokeColor;
                                    }
                                })
                                .style("stroke-width", function (d, i) {
                                    if (self.settings.HISTOGRAMMouseClickStrokeWidth == "none") {
                                        return "";
                                    } else {
                                        return self.settings.HISTOGRAMMouseClickStrokeWidth;
                                    }
                                });
                    })
                }
                if (self.settings.HISTOGRAMMouseDownDisplay == true) {
                    HISTOGRAMMouseOn.on("mousedown", function (d) {
                        d3.select(this)
                                .style("fill", function (d, i) {
                                    if (self.settings.HISTOGRAMMouseDownColor == "none") {
                                        return "";
                                    } else {
                                        return self.settings.HISTOGRAMMouseDownColor;
                                    }
                                })
                                .style("opacity", function (d, i) {
                                    if (self.settings.HISTOGRAMMouseDownOpacity == "none") {
                                        return "";
                                    } else {
                                        return self.settings.HISTOGRAMMouseDownOpacity;
                                    }
                                })
                                .style("stroke", function (d, i) {
                                    if (self.settings.HISTOGRAMMouseDownStrokeColor == "none") {
                                        return "";
                                    } else {
                                        return self.settings.HISTOGRAMMouseDownStrokeColor;
                                    }
                                })
                                .style("stroke-width", function (d, i) {
                                    if (self.settings.HISTOGRAMMouseDownStrokeWidth == "none") {
                                        return "";
                                    } else {
                                        return self.settings.HISTOGRAMMouseDownStrokeWidth;
                                    }
                                });
                    })
                }
                if (self.settings.HISTOGRAMMouseEnterDisplay == true) {
                    HISTOGRAMMouseOn.on("mouseenter", function (d) {
                        d3.select(this)
                                .style("fill", function (d, i) {
                                    if (self.settings.HISTOGRAMMouseEnterColor == "none") {
                                        return "";
                                    } else {
                                        return self.settings.HISTOGRAMMouseEnterColor;
                                    }
                                })
                                .style("opacity", function (d, i) {
                                    if (self.settings.HISTOGRAMMouseEnterOpacity == "none") {
                                        return "";
                                    } else {
                                        return self.settings.HISTOGRAMMouseEnterOpacity;
                                    }
                                })
                                .style("stroke", function (d, i) {
                                    if (self.settings.HISTOGRAMMouseEnterStrokeColor == "none") {
                                        return "";
                                    } else {
                                        return self.settings.HISTOGRAMMouseEnterStrokeColor;
                                    }
                                })
                                .style("stroke-width", function (d, i) {
                                    if (self.settings.HISTOGRAMMouseEnterStrokeWidth == "none") {
                                        return "";
                                    } else {
                                        return self.settings.HISTOGRAMMouseEnterStrokeWidth;
                                    }
                                });
                    })
                }
                if (self.settings.HISTOGRAMMouseLeaveDisplay == true) {
                    HISTOGRAMMouseOn.on("mouseleave", function (d) {
                        ...
                        d3.select(this)
                                .style("fill", function (d, i) {
                                    if (self.settings.HISTOGRAMMouseLeaveColor == "none") {
                                        return "";
                                    } else {
                                        return self.settings.HISTOGRAMMouseLeaveColor;
                                    }
                                })
                                .style("opacity", function (d, i) {
                                    if (self.settings.HISTOGRAMMouseLeaveOpacity == "none") {
                                        return "";
                                    } else {
                                        return self.settings.HISTOGRAMMouseLeaveOpacity;
                                    }
                                })
                                .style("stroke", function (d, i) {
                                    if (self.settings.HISTOGRAMMouseLeaveStrokeColor == "none") {
                                        return "";
                                    } else {
                                        return self.settings.HISTOGRAMMouseLeaveStrokeColor;
                                    }
                                })
                                .style("stroke-width", function (d, i) {
                                    if (self.settings.HISTOGRAMMouseLeaveStrokeWidth == "none") {
                                        return "";
                                    } else {
                                        return self.settings.HISTOGRAMMouseLeaveStrokeWidth;
                                    }
                                });
                    })
                }
                if (self.settings.HISTOGRAMMouseUpDisplay == true) {
                    HISTOGRAMMouseOn.on("mouseup", function (d) {
                        d3.select(this)
                                .style("fill", function (d, i) {
                                    if (self.settings.HISTOGRAMMouseUpColor == "none") {
                                        return "";
                                    } else {
                                        return self.settings.HISTOGRAMMouseUpColor;
                                    }
                                })
                                .style("opacity", function (d, i) {
                                    if (self.settings.HISTOGRAMMouseUpOpacity == "none") {
                                        return "";
                                    } else {
                                        return self.settings.HISTOGRAMMouseUpOpacity;
                                    }
                                })
                                .style("stroke", function (d, i) {
                                    if (self.settings.HISTOGRAMMouseUpStrokeColor == "none") {
                                        return "";
                                    } else {
                                        return self.settings.HISTOGRAMMouseUpStrokeColor;
                                    }
                                })
                                .style("stroke-width", function (d, i) {
                                    if (self.settings.HISTOGRAMMouseUpStrokeWidth == "none") {
                                        return "";
                                    } else {
                                        return self.settings.HISTOGRAMMouseUpStrokeWidth;
                                    }
                                });
                    })
                }
                if (self.settings.HISTOGRAMMouseMoveDisplay == true) {
                    HISTOGRAMMouseOn.on("mousemove", function (d) {
                        d3.select(this)
                                .style("fill", function (d, i) {
                                    if (self.settings.HISTOGRAMMouseMoveColor == "none") {
                                        return "";
                                    } else {
                                        return self.settings.HISTOGRAMMouseMoveColor;
                                    }
                                })
                                .style("opacity", function (d, i) {
                                    if (self.settings.HISTOGRAMMouseMoveOpacity == "none") {
                                        return "";
                                    } else {
                                        return self.settings.HISTOGRAMMouseMoveOpacity;
                                    }
                                })
                                .style("stroke", function (d, i) {
                                    if (self.settings.HISTOGRAMMouseMoveStrokeColor == "none") {
                                        return "";
                                    } else {
                                        return self.settings.HISTOGRAMMouseMoveStrokeColor;
                                    }
                                })
                                .style("stroke-width", function (d, i) {
                                    if (self.settings.HISTOGRAMMouseMoveStrokeWidth == "none") {
                                        return "";
                                    } else {
                                        return self.settings.HISTOGRAMMouseMoveStrokeWidth;
                                    }
                                });
                ...
                if (self.settings.HISTOGRAMMouseOutDisplay == true) {
                    HISTOGRAMMouseOn.on("mouseout", function (d) {
                        ...
                        d3.select(this)
                                ...
                                .style("fill", function (d, i) {
                                    if (self.settings.HISTOGRAMMouseOutColor == "none") {
                                        return "";
                                    } else {
                                        return self.settings.HISTOGRAMMouseOutColor;
                                    }
                                })
                                .style("opacity", function (d, i) {
                                    if (self.settings.HISTOGRAMMouseOutOpacity == "none") {
                                        return "";
                                    } else {
                                        return self.settings.HISTOGRAMMouseOutOpacity;
                                    }
                                })
                                .style("stroke", function (d, i) {
                                    if (self.settings.HISTOGRAMMouseOutStrokeColor == "none") {
                                        return "";
                                    } else {
                                        return self.settings.HISTOGRAMMouseOutStrokeColor;
                                    }
                                })
                                .style("stroke-width", function (d, i) {
                                    if (self.settings.HISTOGRAMMouseOutStrokeWidth == "none") {
                                        return "";
                                    } else {
                                        return self.settings.HISTOGRAMMouseOutStrokeWidth;
                                    }
                                });
                    });
                }
		...
		//if there is a line module
        if (self.LINE.length > 0) {
			//for each line object passed to BioCircos
            for (var linei = 0; linei < self.LINE.length; linei++) {
				...
				if(self.textTrack == true){
                    	svg.append("g")		
                        .attr("class","TextTrackS3")
                        .attr("id","TextTrackS3")
                        .selectAll("path.TextTrack3")
                        .data(TextChord.groups)
                  .enter().append("path")
                        .attr("class","TextTrack3")
                        .attr("id", function(linei){return "TextTrack3" + linei;})            
                        .attr("d", d3.svg.arc().innerRadius((0 + self.LINEsettings.minRadius)).outerRadius((0 + self.LINEsettings.maxRadius)))
                        .style("fill", "none")
                        .style("stroke", "none")
                        .style("stroke-width", "1")
                        .attr("name", function(d) { return d.index+1; })
                ...
                svg.append("text")
				.attr("id", "BioCircosExpressionText")
				.attr("x", 300)
				.attr("dy", +0)
				.attr("font-size", 8)
				.append("textPath")
				.attr("xlink:href", "#TextTrack3")
				.text("Coverage");    
				...
				for (var chri = 0; chri < self.genomeLabel.length; chri++) {
					...
					var line_objects_a_chr = line_objects.filter(function (element, pos) { //filter by the chromosome
                        return element.line_chr == self.genomeLabel[chri]
                    })
					//append a g element to the svg and draw appropriate lines in it to display whatever the line module was passed
                    if (line_objects_a_chr.length != 0) {
                        svg.append("g")
                                ...
                                .attr("d", d3.svg.line() //draw line points here
                                        .x(function (d) {
                                            return d.x;
                                        })
                                        .y(function (d) {
				...
				//ok so line objects works a little differently, it takes the line data pasesed it as values, gets the max and min to get fixed points to draw relative to
				//passes the line object to d3.map and gets the values need to draw lines. 
                function BioCircosLINE(d) {
                    return self.LINE[linei].map(function (v, i) { //d[self.initGenome[v.chr]] is the chromsome number
			...
			//tooltips, we use mouse over and mouse out, same method as other modules
            if (self.settings.LINEMouseEvent == true) {
                ...
                if (self.settings.LINEMouseOverDisplay == true) {
                    LINEMouseOn.on("mouseover", function (d) {
                        ...
                        d3.select(this)
                                .style("opacity", function (d, i) {
                                    if (self.settings.LINEMouseOverLineOpacity == "none") {
                                        return "";
                                    } else {
                                        return self.settings.LINEMouseOverLineOpacity;
                                    }
                                })
                                .style("stroke", function (d, i) {
                                    if (self.settings.LINEMouseOverLineStrokeColor == "none") {
                                        return "";
                                    } else {
                                        return self.settings.LINEMouseOverLineStrokeColor;
                                    }
                                })
                                .style("stroke-width", function (d, i) {
                                    if (self.settings.LINEMouseOverLineStrokeWidth == "none") {
                                        return "";
                                    } else {
                                        return self.settings.LINEMouseOverLineStrokeWidth;
                                    }
                                });
                    })
                }
                if (self.settings.LINEMouseClickDisplay == true) {
                    LINEMouseOn.on("click", function (d) {
                        d3.select(this)
                                .style("opacity", function (d, i) {
                                    if (self.settings.LINEMouseClickLineOpacity == "none") {
                                        return "";
                                    } else {
                                        return self.settings.LINEMouseClickLineOpacity;
                                    }
                                })
                                .style("stroke", function (d, i) {
                                    if (self.settings.LINEMouseClickLineStrokeColor == "none") {
                                        return "";
                                    } else {
                                        return self.settings.LINEMouseClickLineStrokeColor;
                                    }
                                })
                                .style("stroke-width", function (d, i) {
                                    if (self.settings.LINEMouseClickLineStrokeWidth == "none") {
                                        return "";
                                    } else {
                                        return self.settings.LINEMouseClickLineStrokeWidth;
                                    }
                                });
                    })
                }
                if (self.settings.LINEMouseDownDisplay == true) {
                    LINEMouseOn.on("mousedown", function (d) {
                        d3.select(this)
                                .style("opacity", function (d, i) {
                                    if (self.settings.LINEMouseDownLineOpacity == "none") {
                                        return "";
                                    } else {
                                        return self.settings.LINEMouseDownLineOpacity;
                                    }
                                })
                                .style("stroke", function (d, i) {
                                    if (self.settings.LINEMouseDownLineStrokeColor == "none") {
                                        return "";
                                    } else {
                                        return self.settings.LINEMouseDownLineStrokeColor;
                                    }
                                })
                                .style("stroke-width", function (d, i) {
                                    if (self.settings.LINEMouseDownLineStrokeWidth == "none") {
                                        return "";
                                    } else {
                                        return self.settings.LINEMouseDownLineStrokeWidth;
                                    }
                                });
                    })
                }
                if (self.settings.LINEMouseEnterDisplay == true) {
                    LINEMouseOn.on("mouseenter", function (d) {
                        d3.select(this)
                                .style("opacity", function (d, i) {
                                    if (self.settings.LINEMouseEnterLineOpacity == "none") {
                                        return "";
                                    } else {
                                        return self.settings.LINEMouseEnterLineOpacity;
                                    }
                                })
                                .style("stroke", function (d, i) {
                                    if (self.settings.LINEMouseEnterLineStrokeColor == "none") {
                                        return "";
                                    } else {
                                        return self.settings.LINEMouseEnterLineStrokeColor;
                                    }
                                })
                                .style("stroke-width", function (d, i) {
                                    if (self.settings.LINEMouseEnterLineStrokeWidth == "none") {
                                        return "";
                                    } else {
                                        return self.settings.LINEMouseEnterLineStrokeWidth;
                                    }
                                });
                    })
                }
                if (self.settings.LINEMouseLeaveDisplay == true) {
                    LINEMouseOn.on("mouseleave", function (d) {
                        ...
                        d3.select(this)
                                .style("opacity", function (d, i) {
                                    if (self.settings.LINEMouseLeaveLineOpacity == "none") {
                                        return "";
                                    } else {
                                        return self.settings.LINEMouseLeaveLineOpacity;
                                    }
                                })
                                .style("stroke", function (d, i) {
                                    if (self.settings.LINEMouseLeaveLineStrokeColor == "none") {
                                        return "";
                                    } else {
                                        return self.settings.LINEMouseLeaveLineStrokeColor;
                                    }
                                })
                                .style("stroke-width", function (d, i) {
                                    if (self.settings.LINEMouseLeaveLineStrokeWidth == "none") {
                                        return "";
                                    } else {
                                        return self.settings.LINEMouseLeaveLineStrokeWidth;
                                    }
                                });
                    })
                }
                if (self.settings.LINEMouseUpDisplay == true) {
                    LINEMouseOn.on("mouseup", function (d) {
                        d3.select(this)
                                .style("opacity", function (d, i) {
                                    if (self.settings.LINEMouseUpLineOpacity == "none") {
                                        return "";
                                    } else {
                                        return self.settings.LINEMouseUpLineOpacity;
                                    }
                                })
                                .style("stroke", function (d, i) {
                                    if (self.settings.LINEMouseUpLineStrokeColor == "none") {
                                        return "";
                                    } else {
                                        return self.settings.LINEMouseUpLineStrokeColor;
                                    }
                                })
                                .style("stroke-width", function (d, i) {
                                    if (self.settings.LINEMouseUpLineStrokeWidth == "none") {
                                        return "";
                                    } else {
                                        return self.settings.LINEMouseUpLineStrokeWidth;
                                    }
                                });
                    })
                }
                if (self.settings.LINEMouseMoveDisplay == true) {
                    LINEMouseOn.on("mousemove", function (d) {
                        d3.select(this)
                                .style("opacity", function (d, i) {
                                    if (self.settings.LINEMouseMoveLineOpacity == "none") {
                                        return "";
                                    } else {
                                        return self.settings.LINEMouseMoveLineOpacity;
                                    }
                                })
                                .style("stroke", function (d, i) {
                                    if (self.settings.LINEMouseMoveLineStrokeColor == "none") {
                                        return "";
                                    } else {
                                        return self.settings.LINEMouseMoveLineStrokeColor;
                                    }
                                })
                                .style("stroke-width", function (d, i) {
                                    if (self.settings.LINEMouseMoveLineStrokeWidth == "none") {
                                        return "";
                                    } else {
                                        return self.settings.LINEMouseMoveLineStrokeWidth;
                                    }
                                });
                ...
                if (self.settings.LINEMouseOutDisplay == true) {
                    LINEMouseOn.on("mouseout", function (d) {
                        ...
                        d3.select(this)
                                ...
                                .style("opacity", function (d, i) {
                                    if (self.settings.LINEMouseOutLineOpacity == "none") {
                                        return "";
                                    } else {
                                        return self.settings.LINEMouseOutLineOpacity;
                                    }
                                })
                                .style("stroke", function (d, i) {
                                    if (self.settings.LINEMouseOutLineStrokeColor == "none") {
                                        return "";
                                    } else {
                                        return self.settings.LINEMouseOutLineStrokeColor;
                                    }
                                })
                                .style("stroke-width", function (d, i) {
                                    if (self.settings.LINEMouseOutLineStrokeWidth == "none") {
                                        return "";
                                    } else {
                                        return self.settings.LINEMouseOutLineStrokeWidth;
                                    }
                                });
                    });
                }
        ...
        if (self.CNV.length > 0) {
            for (var cnvi = 0; cnvi < self.CNV.length; cnvi++) {
                ...
                svg.append("g")
                        ...
                        .attr("fill", function (d, i) {
                            return self.CNVsettings.CNVColor;
                        })
                        .attr("d", function (d, i) {
                            var cnv = d3.svg.arc().innerRadius(self.CNVsettings.minRadius + d.cnv_deviation).outerRadius(self.CNVsettings.minRadius + self.CNVsettings.CNVwidth + d.cnv_deviation);
                            return cnv(d, i);
                        });
            ...
            if (self.settings.CNVMouseEvent == true) {
                ...
                if (self.settings.CNVMouseOverDisplay == true) {
                    CNVMouseOn.on("mouseover", function (d) {
                        ...
                        d3.select(this)
                                .style("fill", function (d, i) {
                                    if (self.settings.CNVMouseOverColor == "none") {
                                        return "";
                                    } else {
                                        return self.settings.CNVMouseOverColor;
                                    }
                                })
                                .style("opacity", function (d, i) {
                                    if (self.settings.CNVMouseOverArcOpacity == "none") {
                                        return "";
                                    } else {
                                        return self.settings.CNVMouseOverArcOpacity;
                                    }
                                })
                                .style("stroke", function (d, i) {
                                    if (self.settings.CNVMouseOverArcStrokeColor == "none") {
                                        return "";
                                    } else {
                                        return self.settings.CNVMouseOverArcStrokeColor;
                                    }
                                })
                                .style("stroke-width", function (d, i) {
                                    if (self.settings.CNVMouseOverArcStrokeWidth == "none") {
                                        return "";
                                    } else {
                                        return self.settings.CNVMouseOverArcStrokeWidth;
                                    }
                                });
                    })
                }
                ...
                if (self.settings.CNVMouseClickDisplay == true) {
                    CNVMouseOn.on("click", function (d) {
                        d3.select(this)
                                .style("fill", function (d, i) {
                                    if (self.settings.CNVMouseClickColor == "none") {
                                        return "";
                                    } else {
                                        return self.settings.CNVMouseClickColor;
                                    }
                                })
                                .style("opacity", function (d, i) {
                                    if (self.settings.CNVMouseClickArcOpacity == "none") {
                                        return "";
                                    } else {
                                        return self.settings.CNVMouseClickArcOpacity;
                                    }
                                })
                                .style("stroke", function (d, i) {
                                    if (self.settings.CNVMouseClickArcStrokeColor == "none") {
                                        return "";
                                    } else {
                                        return self.settings.CNVMouseClickArcStrokeColor;
                                    }
                                })
                                .style("stroke-width", function (d, i) {
                                    if (self.settings.CNVMouseClickArcStrokeWidth == "none") {
                                        return "";
                                    } else {
                                        return self.settings.CNVMouseClickArcStrokeWidth;
                                    }
                                });
                ...
                if (self.settings.CNVMouseDownDisplay == true) {
                    CNVMouseOn.on("mousedown", function (d) {
                        d3.select(this)
                                .style("fill", function (d, i) {
                                    if (self.settings.CNVMouseDownColor == "none") {
                                        return "";
                                    } else {
                                        return self.settings.CNVMouseDownColor;
                                    }
                                })
                                .style("opacity", function (d, i) {
                                    if (self.settings.CNVMouseDownCircleOpacity == "none") {
                                        return "";
                                    } else {
                                        return self.settings.CNVMouseDownCircleOpacity;
                                    }
                                })
                                .style("stroke", function (d, i) {
                                    if (self.settings.CNVMouseDownCircleStrokeColor == "none") {
                                        return "";
                                    } else {
                                        return self.settings.CNVMouseDownCircleStrokeColor;
                                    }
                                })
                                .style("stroke-width", function (d, i) {
                                    if (self.settings.CNVMouseDownCircleStrokeWidth == "none") {
                                        return "";
                                    } else {
                                        return self.settings.CNVMouseDownCircleStrokeWidth;
                                    }
                                });
                    })
                }
                if (self.settings.CNVMouseEnterDisplay == true) {
                    CNVMouseOn.on("mouseenter", function (d) {
                        d3.select(this)
                                .style("fill", function (d, i) {
                                    if (self.settings.CNVMouseEnterColor == "none") {
                                        return "";
                                    } else {
                                        return self.settings.CNVMouseEnterColor;
                                    }
                                })
                                .style("opacity", function (d, i) {
                                    if (self.settings.CNVMouseEnterCircleOpacity == "none") {
                                        return "";
                                    } else {
                                        return self.settings.CNVMouseEnterCircleOpacity;
                                    }
                                })
                                .style("stroke", function (d, i) {
                                    if (self.settings.CNVMouseEnterCircleStrokeColor == "none") {
                                        return "";
                                    } else {
                                        return self.settings.CNVMouseEnterCircleStrokeColor;
                                    }
                                })
                                .style("stroke-width", function (d, i) {
                                    if (self.settings.CNVMouseEnterCircleStrokeWidth == "none") {
                                        return "";
                                    } else {
                                        return self.settings.CNVMouseEnterCircleStrokeWidth;
                                    }
                                });
                    })
                }
                if (self.settings.CNVMouseLeaveDisplay == true) {
                    CNVMouseOn.on("mouseleave", function (d) {
                        ...
                        d3.select(this)
                                .style("fill", function (d, i) {
                                    if (self.settings.CNVMouseLeaveColor == "none") {
                                        return "";
                                    } else {
                                        return self.settings.CNVMouseLeaveColor;
                                    }
                                })
                                .style("opacity", function (d, i) {
                                    if (self.settings.CNVMouseLeaveCircleOpacity == "none") {
                                        return "";
                                    } else {
                                        return self.settings.CNVMouseLeaveCircleOpacity;
                                    }
                                })
                                .style("stroke", function (d, i) {
                                    if (self.settings.CNVMouseLeaveCircleStrokeColor == "none") {
                                        return "";
                                    } else {
                                        return self.settings.CNVMouseLeaveCircleStrokeColor;
                                    }
                                })
                                .style("stroke-width", function (d, i) {
                                    if (self.settings.CNVMouseLeaveCircleStrokeWidth == "none") {
                                        return "";
                                    } else {
                                        return self.settings.CNVMouseLeaveCircleStrokeWidth;
                                    }
                                });
                    })
                }
                if (self.settings.CNVMouseUpDisplay == true) {
                    CNVMouseOn.on("mouseup", function (d) {
                        d3.select(this)
                                .style("fill", function (d, i) {
                                    if (self.settings.CNVMouseUpColor == "none") {
                                        return "";
                                    } else {
                                        return self.settings.CNVMouseUpColor;
                                    }
                                })
                                .style("opacity", function (d, i) {
                                    if (self.settings.CNVMouseUpCircleOpacity == "none") {
                                        return "";
                                    } else {
                                        return self.settings.CNVMouseUpCircleOpacity;
                                    }
                                })
                                .style("stroke", function (d, i) {
                                    if (self.settings.CNVMouseUpCircleStrokeColor == "none") {
                                        return "";
                                    } else {
                                        return self.settings.CNVMouseUpCircleStrokeColor;
                                    }
                                })
                                .style("stroke-width", function (d, i) {
                                    if (self.settings.CNVMouseUpCircleStrokeWidth == "none") {
                                        return "";
                                    } else {
                                        return self.settings.CNVMouseUpCircleStrokeWidth;
                                    }
                                });
                    })
                }
                if (self.settings.CNVMouseMoveDisplay == true) {
                    CNVMouseOn.on("mousemove", function (d) {
                        d3.select(this)
                                .style("fill", function (d, i) {
                                    if (self.settings.CNVMouseMoveColor == "none") {
                                        return "";
                                    } else {
                                        return self.settings.CNVMouseMoveColor;
                                    }
                                })
                                .style("opacity", function (d, i) {
                                    if (self.settings.CNVMouseMoveCircleOpacity == "none") {
                                        return "";
                                    } else {
                                        return self.settings.CNVMouseMoveCircleOpacity;
                                    }
                                })
                                .style("stroke", function (d, i) {
                                    if (self.settings.CNVMouseMoveCircleStrokeColor == "none") {
                                        return "";
                                    } else {
                                        return self.settings.CNVMouseMoveCircleStrokeColor;
                                    }
                                })
                                .style("stroke-width", function (d, i) {
                                    if (self.settings.CNVMouseMoveCircleStrokeWidth == "none") {
                                        return "";
                                    } else {
                                        return self.settings.CNVMouseMoveCircleStrokeWidth;
                                    }
                                });
                ...
                if (self.settings.CNVMouseOutDisplay == true) {
                    CNVMouseOn.on("mouseout", function (d) {
                        ...
                        d3.select(this)
                                ...
                                .style("fill", function (d, i) {
                                    if (self.settings.CNVMouseOutColor == "none") {
                                        return "";
                                    } else {
                                        return self.settings.CNVMouseOutColor;
                                    }
                                })
                                .style("opacity", function (d, i) {
                                    if (self.settings.CNVMouseOutCircleOpacity == "none") {
                                        return "";
                                    } else {
                                        return self.settings.CNVMouseOutCircleOpacity;
                                    }
                                })
                                .style("stroke", function (d, i) {
                                    if (self.settings.CNVMouseOutCircleStrokeColor == "none") {
                                        return "";
                                    } else {
                                        return self.settings.CNVMouseOutCircleStrokeColor;
                                    }
                                })
                                .style("stroke-width", function (d, i) {
                                    if (self.settings.CNVMouseOutCircleStrokeWidth == "none") {
                                        return "";
                                    } else {
                                        return self.settings.CNVMouseOutCircleStrokeWidth;
                                    }
                                });
                    });
                }
        ...
        //if there is a heatmap argument passed to biocircos
        if (self.HEATMAP.length > 0) {
			//for all the HEatmaps passed to biocircos
            for (var heatmapi = 0; heatmapi < self.HEATMAP.length; heatmapi++) {
                ...
                //type of heatmap (Diff Exp, or Gene Exp)
				var type = heatmap_objects[0].heatmap_name;
                //failed text track
				if(self.textTrack==true){
                    	svg.append("g")		
                        .attr("class","TextTrackS4")
                        .attr("id","TextTrackS4")
                        .selectAll("path.TextTrack4")
                        .data(TextChord.groups)
                  .enter().append("path")
                        .attr("class","TextTrack4")
                        .attr("id", "TextTrack4")            
                        .attr("d", d3.svg.arc().innerRadius((innerRadius + -25)).outerRadius((outerRadius -65)))
                        .style("fill", "none")
                        .style("stroke", "none")
                        .style("stroke-width", "1")
                        .attr("name", function(d) { return d.index+1; })
				...
				//interpolator for the two colours
                var HeatmapValue2Color = d3.interpolate(HeatmapMinColor, HeatmapMaxColor);
				//empty colour if value is 0
                var EmptyColour = d3.interpolate("grey", "green");
				//Heatmap arc function
                ...
                //append a g element and add it to the SVG
				svg.append("g")
                        ...
                        .append("path")
                        .attr("class", "BioCircosHEATMAP")
                        .attr("fill", function (d, i) { //generate fill for this heatmap object, if block is for values of 0
				if (((d.heatmap_value - self.heatmap_value_maxmin(self.HEATMAP[heatmapi])[1])/(self.heatmap_value_maxmin(self.HEATMAP[heatmapi])[0]-self.heatmap_value_maxmin(self.HEATMAP[heatmapi])[1])) === 0){
						return EmptyColour((d.heatmap_value - self.heatmap_value_maxmin(self.HEATMAP[heatmapi])[1])/(self.heatmap_value_maxmin(self.HEATMAP[heatmapi])[0]-self.heatmap_value_maxmin(self.HEATMAP[heatmapi])[1]));	
					...
					.attr("d", function (d, i) { //draw the actual svg
						return heatmap(d, i);
				});
				self.init_HEATMAPsettings();
				//create the HeatMAp objects, pass to d3.Map //d[self.initGenome[v.chr]] is the chromsome number generate the values from the chord.groups
			...
			//tooltips, we use the Mouseover and mouse out, same as above for generating tooltips
			if (self.settings.HEATMAPMouseEvent == true) {
				...
				if (self.settings.HEATMAPMouseOverDisplay == true) {
					HEATMAPMouseOn.on("mouseover", function (d) {
						...
						d3.select(this)
						.style("fill", function (d, i) {
							if (self.settings.HEATMAPMouseOverColor == "none") {
								return "";
							} else {
								return self.settings.HEATMAPMouseOverColor;
							}
						})
						.style("opacity", function (d, i) {
							if (self.settings.HEATMAPMouseOverOpacity == "none") {
								return "";
							} else {
								return self.settings.HEATMAPMouseOverOpacity;
							}
						})
						.style("stroke", function (d, i) {
							if (self.settings.HEATMAPMouseOverStrokeColor == "none") {
								return "";
							} else {
								return self.settings.HEATMAPMouseOverStrokeColor;
							}
						})
						.style("stroke-width", function (d, i) {
							if (self.settings.HEATMAPMouseOverStrokeWidth == "none") {
								return "";
							} else {
								return self.settings.HEATMAPMouseOverStrokeWidth;
							}
						});
					})
				}
				if (self.settings.HEATMAPMouseClickDisplay == true) {
					HEATMAPMouseOn.on("click", function (d) {
						d3.select(this)
						.style("fill", function (d, i) {
							if (self.settings.HEATMAPMouseClickColor == "none") {
								return "";
							} else {
								return self.settings.HEATMAPMouseClickColor;
							}
						})
						.style("opacity", function (d, i) {
							if (self.settings.HEATMAPMouseClickOpacity == "none") {
								return "";
							} else {
								return self.settings.HEATMAPMouseClickOpacity;
							}
						})
						.style("stroke", function (d, i) {
							if (self.settings.HEATMAPMouseClickStrokeColor == "none") {
								return "";
							} else {
								return self.settings.HEATMAPMouseClickStrokeColor;
							}
						})
						.style("stroke-width", function (d, i) {
							if (self.settings.HEATMAPMouseClickStrokeWidth == "none") {
								return "";
							} else {
								return self.settings.HEATMAPMouseClickStrokeWidth;
							}
						});
					})
				}
				if (self.settings.HEATMAPMouseDownDisplay == true) {
					HEATMAPMouseOn.on("mousedown", function (d) {
						d3.select(this)
						.style("fill", function (d, i) {
							if (self.settings.HEATMAPMouseDownColor == "none") {
								return "";
							} else {
								return self.settings.HEATMAPMouseDownColor;
							}
						})
						.style("opacity", function (d, i) {
							if (self.settings.HEATMAPMouseDownOpacity == "none") {
								return "";
							} else {
								return self.settings.HEATMAPMouseDownOpacity;
							}
						})
						.style("stroke", function (d, i) {
							if (self.settings.HEATMAPMouseDownStrokeColor == "none") {
								return "";
							} else {
								return self.settings.HEATMAPMouseDownStrokeColor;
							}
						})
						.style("stroke-width", function (d, i) {
							if (self.settings.HEATMAPMouseDownStrokeWidth == "none") {
								return "";
							} else {
								return self.settings.HEATMAPMouseDownStrokeWidth;
							}
						});
					})
				}
				if (self.settings.HEATMAPMouseEnterDisplay == true) {
					HEATMAPMouseOn.on("mouseenter", function (d) {
						d3.select(this)
						.style("fill", function (d, i) {
							if (self.settings.HEATMAPMouseEnterColor == "none") {
								return "";
							} else {
								return self.settings.HEATMAPMouseEnterColor;
							}
						})
						.style("opacity", function (d, i) {
							if (self.settings.HEATMAPMouseEnterOpacity == "none") {
								return "";
							} else {
								return self.settings.HEATMAPMouseEnterOpacity;
							}
						})
						.style("stroke", function (d, i) {
							if (self.settings.HEATMAPMouseEnterStrokeColor == "none") {
								return "";
							} else {
								return self.settings.HEATMAPMouseEnterStrokeColor;
							}
						})
						.style("stroke-width", function (d, i) {
							if (self.settings.HEATMAPMouseEnterStrokeWidth == "none") {
								return "";
							} else {
								return self.settings.HEATMAPMouseEnterStrokeWidth;
							}
						});
					})
				}
				if (self.settings.HEATMAPMouseLeaveDisplay == true) {
					HEATMAPMouseOn.on("mouseleave", function (d) {
						...
						d3.select(this)
						.style("fill", function (d, i) {
							if (self.settings.HEATMAPMouseLeaveColor == "none") {
								return "";
							} else {
								return self.settings.HEATMAPMouseLeaveColor;
							}
						})
						.style("opacity", function (d, i) {
							if (self.settings.HEATMAPMouseLeaveOpacity == "none") {
								return "";
							} else {
								return self.settings.HEATMAPMouseLeaveOpacity;
							}
						})
						.style("stroke", function (d, i) {
							if (self.settings.HEATMAPMouseLeaveStrokeColor == "none") {
								return "";
							} else {
								return self.settings.HEATMAPMouseLeaveStrokeColor;
							}
						})
						.style("stroke-width", function (d, i) {
							if (self.settings.HEATMAPMouseLeaveStrokeWidth == "none") {
								return "";
							} else {
								return self.settings.HEATMAPMouseLeaveStrokeWidth;
							}
						});
					})
				}
				if (self.settings.HEATMAPMouseUpDisplay == true) {
					HEATMAPMouseOn.on("mouseup", function (d) {
						d3.select(this)
						.style("fill", function (d, i) {
							if (self.settings.HEATMAPMouseUpColor == "none") {
								return "";
							} else {
								return self.settings.HEATMAPMouseUpColor;
							}
						})
						.style("opacity", function (d, i) {
							if (self.settings.HEATMAPMouseUpOpacity == "none") {
								return "";
							} else {
								return self.settings.HEATMAPMouseUpOpacity;
							}
						})
						.style("stroke", function (d, i) {
							if (self.settings.HEATMAPMouseUpStrokeColor == "none") {
								return "";
							} else {
								return self.settings.HEATMAPMouseUpStrokeColor;
							}
						})
						.style("stroke-width", function (d, i) {
							if (self.settings.HEATMAPMouseUpStrokeWidth == "none") {
								return "";
							} else {
								return self.settings.HEATMAPMouseUpStrokeWidth;
							}
						});
					})
				}
				if (self.settings.HEATMAPMouseMoveDisplay == true) {
					HEATMAPMouseOn.on("mousemove", function (d) {
						d3.select(this)
						.style("fill", function (d, i) {
							if (self.settings.HEATMAPMouseMoveColor == "none") {
								return "";
							} else {
								return self.settings.HEATMAPMouseMoveColor;
							}
						})
						.style("opacity", function (d, i) {
							if (self.settings.HEATMAPMouseMoveOpacity == "none") {
								return "";
							} else {
								return self.settings.HEATMAPMouseMoveOpacity;
							}
						})
						.style("stroke", function (d, i) {
							if (self.settings.HEATMAPMouseMoveStrokeColor == "none") {
								return "";
							} else {
								return self.settings.HEATMAPMouseMoveStrokeColor;
							}
						})
						.style("stroke-width", function (d, i) {
							if (self.settings.HEATMAPMouseUpStrokeWidth == "none") {
								...
								return self.settings.HEATMAPMouseMoveStrokeWidth;
							}
						});
				...
				if (self.settings.HEATMAPMouseOutDisplay == true) {
					HEATMAPMouseOn.on("mouseout", function (d) {
						...
						.style("fill", function (d, i) {
							if (self.settings.HEATMAPMouseOutColor == "none") {
								return "";
							} else {
								return self.settings.HEATMAPMouseOutColor;
							}
						})
						.style("opacity", function (d, i) {
							if (self.settings.HEATMAPMouseOutOpacity == "none") {
								return "";
							} else {
								return self.settings.HEATMAPMouseOutOpacity;
							}
						})
						.style("stroke", function (d, i) {
							if (self.settings.HEATMAPMouseOutStrokeColor == "none") {
								return "";
							} else {
								return self.settings.HEATMAPMouseOutStrokeColor;
							}
						})
						.style("stroke-width", function (d, i) {
							if (self.settings.HEATMAPMouseUpStrokeWidth == "none") {
								...
								return self.settings.HEATMAPMouseOutStrokeWidth;
							}
						});
					});
				}
		...
		//we don't use the scatter module
            ...
            for (var scatteri = 0; scatteri < self.SCATTER.length; scatteri++) {
                ...
                if (self.SCATTERsettings.outerPointType == "circle") {
                    ...
                    if (self.settings.SCATTERMouseClickTextFromData == "first") {
                        svg.append("g")
                                ...
                                .attr("class", "dragText")
                                .attr("id", function (d, i) {
                                    return "scatter" + scatteri + "_" + i;
                                })
                                .text(function (d) {
                                    return d.scatter_chr;
                                })
                                .attr("x", -1000)
                                .attr("y", -1000)
                                .style("opacity", 0)
                ...
                if (self.SCATTERsettings.outerPointType == "rect") {
                    svg.append("g")
                            ...
                            .attr("id", "BioCircosSCATTEROut")
                            .attr("x", function (d) {
                                return d.x - self.SCATTERsettings.outerrectWidth / 2;
                            })
                            .attr("y", function (d) {
                                return d.y - self.SCATTERsettings.outerrectHeight / 2;
                            })
                            .attr("fill", self.SCATTERsettings.outerCircleColor)
                ...
                if (self.SCATTERsettings.innerPointType == "rect") {
                    svg.append("g")
                            ...
                            .attr("id", "BioCircosSCATTEROut")  //out
                            .attr("x", function (d) {
                                return d.x - self.SCATTERsettings.innerrectWidth / 2;
                            })
                            .attr("y", function (d) {
                                return d.y - self.SCATTERsettings.innerrectHeight / 2;
                            })
            ...
            if (self.settings.SCATTERMouseEvent == true) {
                    ...
                    SCATTERMouseOn.on("mouseover", function (d) {
                        ...
                        d3.select(this)
                                .style("r", function (d, i) {
                                    if (self.settings.SCATTERMouseOverCircleSize == "none") {
                                        return "";
                                    } else {
                                        return self.settings.SCATTERMouseOverCircleSize;
                                    }
                                })
                                .style("fill", function (d, i) {
                                    if (self.settings.SCATTERMouseOverColor == "none") {
                                        return "";
                                    } else {
                                        return self.settings.SCATTERMouseOverColor;
                                    }
                                })
                                .style("opacity", function (d, i) {
                                    if (self.settings.SCATTERMouseOverCircleOpacity == "none") {
                                        return "";
                                    } else {
                                        return self.settings.SCATTERMouseOverCircleOpacity;
                                    }
                                })
                                .style("stroke", function (d, i) {
                                    if (self.settings.SCATTERMouseOverCircleStrokeColor == "none") {
                                        return "";
                                    } else {
                                        return self.settings.SCATTERMouseOverCircleStrokeColor;
                                    }
                                })
                                .style("stroke-width", function (d, i) {
                                    if (self.settings.SCATTERMouseOverCircleStrokeWidth == "none") {
                                        return "";
                                    } else {
                                        return self.settings.SCATTERMouseOverCircleStrokeWidth;
                                    }
                                });
                    })
                }
                if (self.settings.SCATTERMouseClickDisplay == true) {
                    SCATTERMouseOn.on("click", function (d) {
                        d3.select(this)
                                .style("r", function (d, i) {
                                    if (self.settings.SCATTERMouseClickCircleSize == "none") {
                                        return "";
                                    } else {
                                        return self.settings.SCATTERMouseClickCircleSize;
                                    }
                                })
                                .style("fill", function (d, i) {
                                    if (self.settings.SCATTERMouseClickColor == "none") {
                                        return "";
                                    } else {
                                        return self.settings.SCATTERMouseClickColor;
                                    }
                                })
                                .style("opacity", function (d, i) {
                                    if (self.settings.SCATTERMouseClickCircleOpacity == "none") {
                                        return "";
                                    } else {
                                        return self.settings.SCATTERMouseClickCircleOpacity;
                                    }
                                })
                                .style("stroke", function (d, i) {
                                    if (self.settings.SCATTERMouseClickCircleStrokeColor == "none") {
                                        return "";
                                    } else {
                                        return self.settings.SCATTERMouseClickCircleStrokeColor;
                                    }
                                })
                                .style("stroke-width", function (d, i) {
                                    if (self.settings.SCATTERMouseClickCircleStrokeWidth == "none") {
                                        return "";
                                    } else {
                                        return self.settings.SCATTERMouseClickCircleStrokeWidth;
                                    }
                                });
                ...
                if (self.settings.SCATTERMouseDownDisplay == true) {
                    SCATTERMouseOn.on("mousedown", function (d) {
                        d3.select(this)
                                .style("r", function (d, i) {
                                    if (self.settings.SCATTERMouseDownCircleSize == "none") {
                                        return "";
                                    } else {
                                        return self.settings.SCATTERMouseDownCircleSize;
                                    }
                                })
                                .style("fill", function (d, i) {
                                    if (self.settings.SCATTERMouseDownColor == "none") {
                                        return "";
                                    } else {
                                        return self.settings.SCATTERMouseDownColor;
                                    }
                                })
                                .style("opacity", function (d, i) {
                                    if (self.settings.SCATTERMouseDownCircleOpacity == "none") {
                                        return "";
                                    } else {
                                        return self.settings.SCATTERMouseDownCircleOpacity;
                                    }
                                })
                                .style("stroke", function (d, i) {
                                    if (self.settings.SCATTERMouseDownCircleStrokeColor == "none") {
                                        return "";
                                    } else {
                                        return self.settings.SCATTERMouseDownCircleStrokeColor;
                                    }
                                })
                                .style("stroke-width", function (d, i) {
                                    if (self.settings.SCATTERMouseDownCircleStrokeWidth == "none") {
                                        return "";
                                    } else {
                                        return self.settings.SCATTERMouseDownCircleStrokeWidth;
                                    }
                                });
                    })
                }
                if (self.settings.SCATTERMouseEnterDisplay == true) {
                    SCATTERMouseOn.on("mouseenter", function (d) {
                        d3.select(this)
                                .style("r", function (d, i) {
                                    if (self.settings.SCATTERMouseEnterCircleSize == "none") {
                                        return "";
                                    } else {
                                        return self.settings.SCATTERMouseEnterCircleSize;
                                    }
                                })
                                .style("fill", function (d, i) {
                                    if (self.settings.SCATTERMouseEnterColor == "none") {
                                        return "";
                                    } else {
                                        return self.settings.SCATTERMouseEnterColor;
                                    }
                                })
                                .style("opacity", function (d, i) {
                                    if (self.settings.SCATTERMouseEnterCircleOpacity == "none") {
                                        return "";
                                    } else {
                                        return self.settings.SCATTERMouseEnterCircleOpacity;
                                    }
                                })
                                .style("stroke", function (d, i) {
                                    if (self.settings.SCATTERMouseEnterCircleStrokeColor == "none") {
                                        return "";
                                    } else {
                                        return self.settings.SCATTERMouseEnterCircleStrokeColor;
                                    }
                                })
                                .style("stroke-width", function (d, i) {
                                    if (self.settings.SCATTERMouseEnterCircleStrokeWidth == "none") {
                                        return "";
                                    } else {
                                        return self.settings.SCATTERMouseEnterCircleStrokeWidth;
                                    }
                                });
                    })
                }
                if (self.settings.SCATTERMouseLeaveDisplay == true) {
                    SCATTERMouseOn.on("mouseleave", function (d) {
                        ...
                        d3.select(this)
                                .style("r", function (d, i) {
                                    if (self.settings.SCATTERMouseLeaveCircleSize == "none") {
                                        return "";
                                    } else {
                                        return self.settings.SCATTERMouseLeaveCircleSize;
                                    }
                                })
                                .style("fill", function (d, i) {
                                    if (self.settings.SCATTERMouseLeaveColor == "none") {
                                        return "";
                                    } else {
                                        return self.settings.SCATTERMouseLeaveColor;
                                    }
                                })
                                .style("opacity", function (d, i) {
                                    if (self.settings.SCATTERMouseLeaveCircleOpacity == "none") {
                                        return "";
                                    } else {
                                        return self.settings.SCATTERMouseLeaveCircleOpacity;
                                    }
                                })
                                .style("stroke", function (d, i) {
                                    if (self.settings.SCATTERMouseLeaveCircleStrokeColor == "none") {
                                        return "";
                                    } else {
                                        return self.settings.SCATTERMouseLeaveCircleStrokeColor;
                                    }
                                })
                                .style("stroke-width", function (d, i) {
                                    if (self.settings.SCATTERMouseLeaveCircleStrokeWidth == "none") {
                                        return "";
                                    } else {
                                        return self.settings.SCATTERMouseLeaveCircleStrokeWidth;
                                    }
                                });
                    })
                }
                if (self.settings.SCATTERMouseUpDisplay == true) {
                    SCATTERMouseOn.on("mouseup", function (d) {
                        d3.select(this)
                                .style("r", function (d, i) {
                                    if (self.settings.SCATTERMouseUpCircleSize == "none") {
                                        return "";
                                    } else {
                                        return self.settings.SCATTERMouseUpCircleSize;
                                    }
                                })
                                .style("fill", function (d, i) {
                                    if (self.settings.SCATTERMouseUpColor == "none") {
                                        return "";
                                    } else {
                                        return self.settings.SCATTERMouseUpColor;
                                    }
                                })
                                .style("opacity", function (d, i) {
                                    if (self.settings.SCATTERMouseUpCircleOpacity == "none") {
                                        return "";
                                    } else {
                                        return self.settings.SCATTERMouseUpCircleOpacity;
                                    }
                                })
                                .style("stroke", function (d, i) {
                                    if (self.settings.SCATTERMouseUpCircleStrokeColor == "none") {
                                        return "";
                                    } else {
                                        return self.settings.SCATTERMouseUpCircleStrokeColor;
                                    }
                                })
                                .style("stroke-width", function (d, i) {
                                    if (self.settings.SCATTERMouseUpCircleStrokeWidth == "none") {
                                        return "";
                                    } else {
                                        return self.settings.SCATTERMouseUpCircleStrokeWidth;
                                    }
                                });
                    })
                }
                if (self.settings.SCATTERMouseMoveDisplay == true) {
                    SCATTERMouseOn.on("mousemove", function (d) {
                        d3.select(this)
                                .style("r", function (d, i) {
                                    if (self.settings.SCATTERMouseMoveCircleSize == "none") {
                                        return "";
                                    } else {
                                        return self.settings.SCATTERMouseMoveCircleSize;
                                    }
                                })
                                .style("fill", function (d, i) {
                                    if (self.settings.SCATTERMouseMoveColor == "none") {
                                        return "";
                                    } else {
                                        return self.settings.SCATTERMouseMoveColor;
                                    }
                                })
                                .style("opacity", function (d, i) {
                                    if (self.settings.SCATTERMouseMoveCircleOpacity == "none") {
                                        return "";
                                    } else {
                                        return self.settings.SCATTERMouseMoveCircleOpacity;
                                    }
                                })
                                .style("stroke", function (d, i) {
                                    if (self.settings.SCATTERMouseMoveCircleStrokeColor == "none") {
                                        return "";
                                    } else {
                                        return self.settings.SCATTERMouseMoveCircleStrokeColor;
                                    }
                                })
                                .style("stroke-width", function (d, i) {
                                    if (self.settings.SCATTERMouseMoveCircleStrokeWidth == "none") {
                                        return "";
                                    } else {
                                        return self.settings.SCATTERMouseMoveCircleStrokeWidth;
                                    }
                                });
                ...
                if (self.settings.SCATTERMouseOutDisplay == true) {
                    SCATTERMouseOn.on("mouseout", function (d) {
                        ...
                        d3.select(this)
                                ...
                                .style("r", function (d, i) {
                                    if (self.settings.SCATTERMouseOutCircleSize == "none") {
                                        return "";
                                    } else {
                                        return self.settings.SCATTERMouseOutCircleSize;
                                    }
                                })
                                .style("fill", function (d, i) {
                                    if (self.settings.SCATTERMouseOutColor == "none") {
                                        return "";
                                    } else {
                                        return self.settings.SCATTERMouseOutColor;
                                    }
                                })
                                .style("opacity", function (d, i) {
                                    if (self.settings.SCATTERMouseOutCircleOpacity == "none") {
                                        return "";
                                    } else {
                                        return self.settings.SCATTERMouseOutCircleOpacity;
                                    }
                                })
                                .style("stroke", function (d, i) {
                                    if (self.settings.SCATTERMouseOutCircleStrokeColor == "none") {
                                        return "";
                                    } else {
                                        return self.settings.SCATTERMouseOutCircleStrokeColor;
                                    }
                                })
                                .style("stroke-width", function (d, i) {
                                    if (self.settings.SCATTERMouseOutCircleStrokeWidth == "none") {
                                        return "";
                                    } else {
                                        return self.settings.SCATTERMouseOutCircleStrokeWidth;
                                    }
                                });
                    });
                }
            ...
            if (self.settings.SNPMouseEvent == true) {
                ...
                if (self.settings.SNPMouseClickDisplay == true) {
                    SNPMouseOn.on("click", function (d) {
                        d3.select(this)
                                .style("r", function (d, i) {
                                    if (self.settings.SNPMouseClickCircleSize == "none") {
                                        return "";
                                    } else {
                                        return self.settings.SNPMouseClickCircleSize;
                                    }
                                })
                                .style("fill", function (d, i) {
                                    if (self.settings.SNPMouseClickColor == "none") {
                                        return "";
                                    } else {
                                        return self.settings.SNPMouseClickColor;
                                    }
                                })
                                .style("opacity", function (d, i) {
                                    if (self.settings.SNPMouseClickCircleOpacity == "none") {
                                        return "";
                                    } else {
                                        return self.settings.SNPMouseClickCircleOpacity;
                                    }
                                })
                                .style("stroke", function (d, i) {
                                    if (self.settings.SNPMouseClickCircleStrokeColor == "none") {
                                        return "";
                                    } else {
                                        return self.settings.SNPMouseClickCircleStrokeColor;
                                    }
                                })
                                .style("stroke-width", function (d, i) {
                                    if (self.settings.SNPMouseClickCircleStrokeWidth == "none") {
                                        return "";
                                    } else {
                                        return self.settings.SNPMouseClickCircleStrokeWidth;
                                    }
                                });
		...
		//if there is a link Module		
        if (self.LINK.length > 0) {
			//for the link module 
			for (var linki = 0; linki < self.LINK.length; linki++) {
                self.update_LINKsettings(self.LINKConfig[linki]);
				//create the link objects, that contain all the data needed to draw the LinkModules 
                var link_objects = BioCircosLINK(chord.groups())
				//we don't use this, it draws a great big ugly circle around the link module and cuts it short of the chromosome
                if (self.LINKsettings.displayLinkAxis == true) {
                    svg.append("g")
                            ...
                            .attr("r", function (d) {
                                return self.LINKsettings.LinkRadius + self.LINKsettings.LinkAxisPad;
                            });
                }
				//Link SVG draws the link modules to the SVG, appending the g element to the svg and putting the link paths into a path element                
				var Link_svg = svg.append("g")
                        ...
                        .enter().append("path")
                        .attr("d", function (d) {
                            return "M" + d.link_X1 + "," + d.link_Y1 + " Q0,0 " + d.link_X2 + "," + d.link_Y2 + "";	//link paty d.link_* is taken from link objects calculated below
                        })
                        .attr("class", "BioCircosLINK")
                        .attr("fill", "none")
                        .attr("stroke", self.LINKsettings.LinkFillColor)
                        .attr("stroke-width", self.LINKsettings.LinkWidth);
				//we don't use this appends a huge ugly text label onto the svg for each link
				...
				//create link objects from chord.groups and the link object passed to BioCircos
                function BioCircosLINK(d) {
                    return self.LINK[linki].map(function (v, i) { //pass to map to split the objects by v
                        //get the angles for the start and ends of the links
						//d[self.initGenome[v.chr]] is the chromsome number
                        ...
                        return { //all generated in the correct form for PAths
			...
			//same tooltips as above, div, populated by jquery .html(), we use mouse over and mouse out
            if (self.settings.LINKMouseEvent == true) {
                ...
                if (self.settings.LINKMouseOverDisplay == true) {
                    LINKMouseOn.on("mouseover", function (d) {
                        ...
                        d3.select(this)
                                .style("opacity", function (d, i) {
                                    if (self.settings.LINKMouseOverOpacity == "none") {
                                        return "";
                                    } else {
                                        return self.settings.LINKMouseOverOpacity;
                                    }
                                })
                                .style("stroke", function (d, i) {
                                    if (self.settings.LINKMouseOverStrokeColor == "none") {
                                        return "";
                                    } else {
                                        return self.settings.LINKMouseOverStrokeColor;
                                    }
                                })
                                .style("stroke-width", function (d, i) {
                                    if (self.settings.LINKMouseOverStrokeWidth == "none") {
                                        return "";
                                    } else {
                                        return self.settings.LINKMouseOverStrokeWidth;
                                    }
                                });
                    })
                }
                if (self.settings.LINKMouseClickDisplay == true) {
                    LINKMouseOn.on("click", function (d) {
                        d3.select(this)
                                .style("opacity", function (d, i) {
                                    if (self.settings.LINKMouseClickOpacity == "none") {
                                        return "";
                                    } else {
                                        return self.settings.LINKMouseClickOpacity;
                                    }
                                })
                                .style("stroke", function (d, i) {
                                    if (self.settings.LINKMouseClickStrokeColor == "none") {
                                        return "";
                                    } else {
                                        return self.settings.LINKMouseClickStrokeColor;
                                    }
                                })
                                .style("stroke-width", function (d, i) {
                                    if (self.settings.LINKMouseClickStrokeWidth == "none") {
                                        return "";
                                    } else {
                                        return self.settings.LINKMouseClickStrokeWidth;
                                    }
                                });
                    })
                }
                if (self.settings.LINKMouseDownDisplay == true) {
                    LINKMouseOn.on("mousedown", function (d) {
                        d3.select(this)
                                .style("opacity", function (d, i) {
                                    if (self.settings.LINKMouseDownOpacity == "none") {
                                        return "";
                                    } else {
                                        return self.settings.LINKMouseDownOpacity;
                                    }
                                })
                                .style("stroke", function (d, i) {
                                    if (self.settings.LINKMouseDownStrokeColor == "none") {
                                        return "";
                                    } else {
                                        return self.settings.LINKMouseDownStrokeColor;
                                    }
                                })
                                .style("stroke-width", function (d, i) {
                                    if (self.settings.LINKMouseDownStrokeWidth == "none") {
                                        return "";
                                    } else {
                                        return self.settings.LINKMouseDownStrokeWidth;
                                    }
                                });
                    })
                }
                if (self.settings.LINKMouseEnterDisplay == true) {
                    LINKMouseOn.on("mouseenter", function (d) {
                        d3.select(this)
                                .style("opacity", function (d, i) {
                                    if (self.settings.LINKMouseEnterOpacity == "none") {
                                        return "";
                                    } else {
                                        return self.settings.LINKMouseEnterOpacity;
                                    }
                                })
                                .style("stroke", function (d, i) {
                                    if (self.settings.LINKMouseEnterStrokeColor == "none") {
                                        return "";
                                    } else {
                                        return self.settings.LINKMouseEnterStrokeColor;
                                    }
                                })
                                .style("stroke-width", function (d, i) {
                                    if (self.settings.LINKMouseEnterStrokeWidth == "none") {
                                        return "";
                                    } else {
                                        return self.settings.LINKMouseEnterStrokeWidth;
                                    }
                                });
                    })
                }
                if (self.settings.LINKMouseLeaveDisplay == true) {
                    LINKMouseOn.on("mouseleave", function (d) {
                        ...
                        d3.select(this)
                                .style("opacity", function (d, i) {
                                    if (self.settings.LINKMouseLeaveOpacity == "none") {
                                        return "";
                                    } else {
                                        return self.settings.LINKMouseLeaveOpacity;
                                    }
                                })
                                .style("stroke", function (d, i) {
                                    if (self.settings.LINKMouseLeaveStrokeColor == "none") {
                                        return "";
                                    } else {
                                        return self.settings.LINKMouseLeaveStrokeColor;
                                    }
                                })
                                .style("stroke-width", function (d, i) {
                                    if (self.settings.LINKMouseLeaveStrokeWidth == "none") {
                                        return "";
                                    } else {
                                        return self.settings.LINKMouseLeaveStrokeWidth;
                                    }
                                });
                    })
                }
                if (self.settings.LINKMouseUpDisplay == true) {
                    LINKMouseOn.on("mouseup", function (d) {
                        d3.select(this)
                                .style("opacity", function (d, i) {
                                    if (self.settings.LINKMouseUpOpacity == "none") {
                                        return "";
                                    } else {
                                        return self.settings.LINKMouseUpOpacity;
                                    }
                                })
                                .style("stroke", function (d, i) {
                                    if (self.settings.LINKMouseUpStrokeColor == "none") {
                                        return "";
                                    } else {
                                        return self.settings.LINKMouseUpStrokeColor;
                                    }
                                })
                                .style("stroke-width", function (d, i) {
                                    if (self.settings.LINKMouseUpStrokeWidth == "none") {
                                        return "";
                                    } else {
                                        return self.settings.LINKMouseUpStrokeWidth;
                                    }
                                });
                    })
                }
                if (self.settings.LINKMouseMoveDisplay == true) {
                    LINKMouseOn.on("mousemove", function (d) {
                        d3.select(this)
                                .style("opacity", function (d, i) {
                                    if (self.settings.LINKMouseMoveOpacity == "none") {
                                        return "";
                                    } else {
                                        return self.settings.LINKMouseMoveOpacity;
                                    }
                                })
                                .style("stroke", function (d, i) {
                                    if (self.settings.LINKMouseMoveStrokeColor == "none") {
                                        return "";
                                    } else {
                                        return self.settings.LINKMouseMoveStrokeColor;
                                    }
                                })
                                .style("stroke-width", function (d, i) {
                                    if (self.settings.LINKMouseMoveStrokeWidth == "none") {
                                        return "";
                                    } else {
                                        return self.settings.LINKMouseMoveStrokeWidth;
                                    }
                                });
                ...
                if (self.settings.LINKMouseOutDisplay == true) {
                    LINKMouseOn.on("mouseout", function (d) {
                        ...
                        d3.select(this)
                                ...
                                .style("opacity", function (d, i) {
                                    if (self.settings.LINKMouseOutOpacity == "none") {
                                        return "";
                                    } else {
                                        return self.settings.LINKMouseOutOpacity;
                                    }
                                })
                                .style("stroke", function (d, i) {
                                    if (self.settings.LINKMouseOutStrokeColor == "none") {
                                        return "";
                                    } else {
                                        return self.settings.LINKMouseOutStrokeColor;
                                    }
                                })
                                .style("stroke-width", function (d, i) {
                                    if (self.settings.LINKMouseOutStrokeWidth == "none") {
                                        return "";
                                    } else {
                                        return self.settings.LINKMouseOutStrokeWidth;
                                    }
                                });
                    });
                }
	...
	//single chromsome view fucntion, called in the genomemouseon.on "click" function
    BioCircos.prototype.chromview = function (genome, d) {
		//initialise global self variable
		var self = this;
		
		var labeli = 0;
		var ChrominitGenome = new Object();

		var labelInit = self.genomeLabel[d.index]; //the genome label
		ChrominitGenome[labelInit] = labeli;
		//create a chrominitgenome, where the chromsome name is equal to 0 in a key value pair
		//d.index is passed from the on click function data, which is the chord.group values for the selected chromsome
		//create the chord
		var chord = d3.layout.chord()
				.padding(self.chrPad)
				.sortSubgroups(d3.descending)
				.matrix(genome);
		//svg settings for this new views
		var width = self.svgWidth,
				height = self.svgHeight,
				innerRadius = self.innerRadius,
				outerRadius = self.outerRadius;
		//fiil for this new chromosome
		var fill = d3.scale.ordinal()
				.domain(d3.range(4))
				.range(self.genomeFillColor);
		...
		console.log(genome[d.index][0])
		 //create new array suitable for d3.layout.chord
		var ArrayCeption = new Array();
		ArrayCeption.push(genome[d.index][0])
		//single Array is the array that contains the array with genome length in it
		var SingleArray = new Array();
		SingleArray.push(ArrayCeption);
		console.log(SingleArray);
		//create a single chord to draw one big arc
		var SingleChord = d3.layout.chord()
				.padding(0.001)
				.sortSubgroups(d3.descending)
				.matrix(SingleArray);
		...
		//zoom appends a new svg to the same div as the Overall BioCircos, with zoom and panning capabilities
		if (self.zoom == true) {
			var svg2 = d3.select("#MadBadSVG")
					.call(
							d3.behavior.zoom()
							.scaleExtent([0.5, 30])
							.on("zoom", zoom)
			...
			function zoom() {
				a = d3.event.translate[0] + width / 2
				b = d3.event.translate[1] + height / 2
				svg2.attr("transform", "translate("
						+ a + "," + b
						+ ")scale(" + d3.event.scale + ")");
			}
		} else { // append a  new svg without the ability to zoom and pan
			var svg2 = d3.select("#MadBadSVG")
					.append("g")
					.attr("id", "SingleChrom")
					.attr("transform", "translate(" + width / 2 + "," + height / 2 + ")");
		...
		//create the exit circle in the centre of the svg
		var ExitCircle = svg2.append("circle")
				.attr("class", "ThatIsAClassCircle")
				.attr("cx", 0)
				.attr("cy", 0)
				.attr("r", 10)
				.attr("fill", "#FF69B4")
		;
		//append the exit text
		var ExitText = svg2.append("text")
				.attr("class", "ThatIsSomeClassText")
				.attr("x", 0)
				.attr("y","0")
				.attr("dy", 6)
				.style("text-anchor", "middle")
				.text("BACK");
		console.log(BigBioCircos.active);
		
		//exit function for clicking the text
		var ExitMouseOn = d3.selectAll("text.ThatIsSomeClassText");
		//on click, destroy the current chromosome view svg, and swith main biocircos. active back true, setting opacity to 1
		ExitMouseOn.on("click", function (d) {
			d3.select("#SingleChrom").remove();
			
			var active = BigBioCircos.active ? false : true,
					newOpacity = active ? 0 : 1;

			d3.select("#BigBioCircos").style("opacity", "1");
			d3.select("#MadBadSVG")
					.call(
							d3.behavior.zoom()
							.scaleExtent([0.5, 30])
							...
							//reset the zoom function on the main biocircos
			function zoom() {
				var svg = d3.select("#BigBioCircos").attr("transform", "translate(" + width / 2 + "," + height / 2 + ")");
				;
				a = d3.event.translate[0] + width / 2
				b = d3.event.translate[1] + height / 2
				svg.attr("transform", "translate("
		...
		//appends a g element to the new svg with the single chromosome arc path, and a border
		if (self.genomeBorderDisplay == true) {
			svg2.append("g")
					.attr("class", "BioCircosGenome")
					.selectAll("path.BioCircosGenome")
					.data(SingleChord.groups)
					.enter().append("path")
					.attr("class", "BioCircosGenome")
					.style("fill", function (d) {
						return fill(d.index);
		...
		} else {//same as above but without border
			svg2.append("g").selectAll("path.BioCircosGenome")
					.attr("class", "BioCircosGenome")
					.data(SingleChord.groups)
					.enter().append("path")
					.style("fill", function (d) {
		...
		//appends the chromosome name to the midpoint of the arc
		if (self.genomeTextDisplay == true) {
			...
			svg2.append("g").selectAll("text")
					...
					.each(function (d, i) {
						...
						d.name = self.genomeLabel[labelno].substring(self.genomeLabel[labelno].length -2,self.genomeLabel[labelno].length);
					})
		...
		//appends ticks with width of 10 million
		if (self.ticksDisplay == true) {
			var ticks = svg2.append("g").selectAll("g")
					.data(SingleChord.groups)
					.enter().append("g").selectAll("g")
					.data(groupTicks) //group ticks is a data group generated below,
					.enter().append("g")
					.attr("transform", function (d) {
						return "rotate(" + (d.angle * 180 / Math.PI - 90) + ")" + "translate(" + (outerRadius - 0) + ",0)";
					});
			...
			function groupTicks(d) { // calculates start and stop angle for ticks, based on the tickscale
				var k = (d.endAngle - d.startAngle) / d.value;
				console.log(k);
		...
		if (self.settings.GenomeMouseEvent == true) {
			...
			var GenomeMouseOnTooltip = d3.select("body")
					...
					.style("opacity", 0);
			//function for the mouse on, select all paths with class of BioCircosGenome
			var GenomeMouseOn = svg2.selectAll("path.BioCircosGenome");
			...
			if (self.settings.GenomeMouseOverDisplay == true) {
				...
				GenomeMouseOn.on("mouseover", function (d) {
					...
					//set the styles for this mouseover
							.style("fill", function (d, i) {
								if (self.settings.GenomeMouseOverColor == "none") { //chromosome highlight colour
									return "";
								} else {
									return self.settings.GenomeMouseOverColor;
								}
		...
		//Now for the Arc module, with some differences from the Big circos arc
		//if theres an arc module
		if (self.ARC.length > 0) {
			//for all of that arc module arrays passed
			for (var arci = 0; arci < self.ARC.length; arci++) {
				self.update_ARCsettings(self.ARCConfig[arci]);
				//create a reg expression to filter out only arc values form the arc object relating to this chrosomome
				var reg = new RegExp(self.genomeLabel[d.index]);
				console.log(reg);
				var ChromArc = new Array();
				var arc = d3.svg.arc()
						.innerRadius(innerRadius + self.ARCsettings.innerRadius)
						.outerRadius(outerRadius + self.ARCsettings.outerRadius);

				console.log(self.ARC);
				//perform reg exp test
				var ArrayCeption = new Array();
				for (var r = 0; r < self.ARC[arci].length; r++) {
					if (reg.test(self.ARC[arci][r].chr)) {
						ArrayCeption.push(self.ARC[arci][r]);
					}
				}
				
				console.log(ArrayCeption);
				//push array containing all chromosome relevant points to another array
				ChromArc.push(ArrayCeption);

			
				 //create the arc_objects object value array calling BioCIrcos Arc function
				 //conatins all data needed to draw arcs
				var arc_objects = BioCircosArc(SingleChord.groups());			
				//this function takes the arc array, and passes it to D3.map which splits the array entry, and allows you to return a mixture of chord.groupadata and arc object data
				//this can be used to draw an arc in the right place for each chromosome, and the properties for that tiny arc
				//d[ChrominitGenome[v.chr]] is the chromsome number
				function BioCircosArc(d) {
					return ChromArc[arci].map(function (v, i) {
						var arc_k = (d[ChrominitGenome[v.chr]].endAngle - d[ChrominitGenome[v.chr]].startAngle) / d[ChrominitGenome[v.chr]].value;
						return {
							startA: d.startAngle,
							EndA: d.endAngle,
							arc_k: arc_k,
							startAngle: v.start * arc_k + d[ChrominitGenome[v.chr]].startAngle,
							endAngle: v.end * arc_k + d[ChrominitGenome[v.chr]].startAngle,
							arc_chr: v.chr,
							arc_start: v.start,
							arc_end: v.end,
							arc_color: v.color,
				...
				//draw the arcs onto the svg by appoend a g and adding all ther paths from d3.svg.arc into it
				svg2.append("g")
						.attr("class", "BioCircosARC")
						.selectAll("path.BioCircosARC")
						.data(arc_objects)
			...
			if (self.settings.ARCMouseEvent == true) {
				...
				var ARCMouseOn = svg2.selectAll("path.BioCircosARC");
				if (self.settings.ARCMouseOverDisplay == true) {
					ARCMouseOn.on("mouseover", function (d) {
						ARCMouseOnTooltip.html(self.settings.ARCMouseOverTooltipsHtml01 + d.arc_chr + self.settings.ARCMouseOverTooltipsHtml02 + d.arc_start + self.settings.ARCMouseOverTooltipsHtml03 + d.arc_end + self.settings.ARCMouseOverTooltipsHtml04 + d.arc_des)
								.style("left", (d3.event.pageX) + "px")
								.style("top", (d3.event.pageY + 20) + "px")
								.style("position", self.settings.ARCMouseOverTooltipsPosition)
								.style("background-color", self.settings.ARCMouseOverTooltipsBackgroundColor)
	  ...
	  console.log("finished ARC");  }

		//now for SNP Density HISTOGRAM
		//if there is a histogram
		if (self.HISTOGRAM.length > 0) {
			var ChromHist = new Array();
			var ArrayCeptionHIST = new Array();
			//for all elements in that histogram
			for (var histogrami = 0; histogrami < self.HISTOGRAM.length; histogrami++) {
				//update the histogram settings
				self.update_HISTOGRAMsettings(self.HISTOGRAMConfig[histogrami]);
				//create new reg exp with chosen chromosome as pattern
				var reg = new RegExp(self.genomeLabel[d.index]);
				//for all the elements in the array
				
				for (var r = 0; r < self.HISTOGRAM[histogrami].length; r++) {
					//if they are for the right chromosome add them to ArrayCeptionHIST
					if (reg.test(self.HISTOGRAM[histogrami][r].chr)) {
						ArrayCeptionHIST.push(self.HISTOGRAM[histogrami][r]);
					}
				}
				//add arrayCeptionHIst to chrom Hist to get them in the right format
				ChromHist.push(ArrayCeptionHIST);
			//create histogram objects, same way as arc objects, contains all necessary data to draw the histogram
				var histogram_objects = BioCircosHISTOGRAM(SingleChord.groups());

				//append a g element, draw the histogram into it, using d3.svg.arc
				svg2.append("g")
						.attr("class", "BioCircosHISTOGRAMSingle")
						.selectAll("path.BioCircosHISTOGRAM")
						.data(histogram_objects)
						.enter()
						.append("path")
						...
						.attr("d", d3.svg.arc().innerRadius(self.HISTOGRAMsettings.minRadius).outerRadius(function (d) {
							return self.HISTOGRAMsettings.minRadius + ((d.histogram_value - self.histogram_value_maxmin(ChromHist[histogrami])[1]) * (self.HISTOGRAMsettings.maxRadius - self.HISTOGRAMsettings.minRadius) / (self.histogram_value_maxmin(ChromHist[histogrami])[0] - self.histogram_value_maxmin(ChromHist[histogrami])[1]));
						}));
				//BioCircosHISTOGRAM creates the histogram objects by for each chord data passing the BioCircos Histogram data points to a map, and creating key value object for each entry 
				//in the histogram array and returning them to D3.svg.arc to draw and then sets the properties accordingly
				function BioCircosHISTOGRAM(d) {
					return ChromHist[histogrami].map(function (v, i) {
						var histogram_k = (d[ChrominitGenome[v.chr]].endAngle - d[ChrominitGenome[v.chr]].startAngle) / d[ChrominitGenome[v.chr]].value;
						return {
							startAngle: v.start * histogram_k + d[ChrominitGenome[v.chr]].startAngle,
							endAngle: v.end * histogram_k + d[ChrominitGenome[v.chr]].startAngle,
							histogram_chr: v.chr,
							histogram_start: v.start,
							histogram_end: v.end,
							histogram_name: v.name,
			...
			if (self.settings.HISTOGRAMMouseEvent == true) {
				...
				var HISTOGRAMMouseOnTooltip = d3.select("body")
						...
						.attr("id", "BioCircosHISTOGRAMTooltip")
						.style("opacity", 0);
				//select all BioCircosHistogram objects rendered, we are only using Mouseover and mouseout
				var HISTOGRAMMouseOn = svg2.selectAll("path.BioCircosHISTOGRAM");
				//works as before, using jquery.html, we only use mouse over and mouse out
				if (self.settings.HISTOGRAMMouseOverDisplay == true) {
					HISTOGRAMMouseOn.on("mouseover", function (d) {
						HISTOGRAMMouseOnTooltip.html(self.settings.HISTOGRAMMouseOverTooltipsHtml01 + d.histogram_chr + self.settings.HISTOGRAMMouseOverTooltipsHtml02 + d.histogram_start + self.settings.HISTOGRAMMouseOverTooltipsHtml03 + d.histogram_end + self.settings.HISTOGRAMMouseOverTooltipsHtml04 + d.histogram_name + self.settings.HISTOGRAMMouseOverTooltipsHtml05 + d.histogram_value + self.settings.HISTOGRAMMouseOverTooltipsHtml06 + d.histogram_snp)
		...
		//now for  Expression HEATMAP
		//if there is a heatmap argument passed to biocircos
		if (self.HEATMAP.length > 0) {
			//initialise chromheat to store heatmap values for this chromosome
			var ChromHeat = new Array();
			//for all the HEatmaps passed to biocircos
			for (var heatmapi = 0; heatmapi < self.HEATMAP.length; heatmapi++) {
				self.update_HEATMAPsettings(self.HEATMAPConfig[heatmapi]);
				
				var ArrayCeptionHEAT = new Array();
				//new regex, which is chromsome name
				var reg = new RegExp(self.genomeLabel[d.index]);
				//for all the elements in the heatmap add them to ArrayCeptionHEAT if they match the regexp chromosome
				for (var r = 0; r < self.HEATMAP[heatmapi].length; r++) {
					if (reg.test(self.HEATMAP[heatmapi][r].chr)) {
						ArrayCeptionHEAT.push(self.HEATMAP[heatmapi][r]);
					}
				}
				//push arrayceptionHEAT to chromHeat so it's in the right format for multiple heatmaps
				ChromHeat.push(ArrayCeptionHEAT);
				////create heatmap objects
				var heatmap_objects = BioCircosHeatmap(SingleChord.groups());



				//max colour heatmap
				var HeatmapMaxColor = d3.rgb(self.HEATMAPsettings.maxColor);
				//min colour for heatmap both user set
				var HeatmapMinColor = d3.rgb(self.HEATMAPsettings.minColor);
				//empty colour for heatmpas with value of 0
				var HeatmapValue2Color = d3.interpolate(HeatmapMinColor, HeatmapMaxColor);
				var EmptyColour = d3.interpolate("grey", "magenta");
				//heatmap drawing d3.svg.arc call
				var heatmap = d3.svg.arc().innerRadius(innerRadius + self.HEATMAPsettings.innerRadius).outerRadius(outerRadius + self.HEATMAPsettings.outerRadius);
				//append heatmap g element to svg and add all the heatmap arcs to this element
				svg2.append("g")
				.attr("class", "BioCircosHEATMAP")
				.selectAll("path.BioCircosHEATMAP")
				.data(heatmap_objects)
				...
				.attr("fill", function (d, i) {//generate fill for this heatmap object if block is for values of 0
						...
						if (((d.heatmap_value - self.heatmap_value_maxmin(ChromHeat[heatmapi])[1])/(self.heatmap_value_maxmin(ChromHeat[heatmapi])[0]-self.heatmap_value_maxmin(ChromHeat[heatmapi])[1])) === 0){
								return EmptyColour((d.heatmap_value - self.heatmap_value_maxmin(ChromHeat[heatmapi])[1])/(self.heatmap_value_maxmin(ChromHeat[heatmapi])[0]-self.heatmap_value_maxmin(ChromHeat[heatmapi])[1]));	

							} else {
								return HeatmapValue2Color((d.heatmap_value - self.heatmap_value_maxmin(ChromHeat[heatmapi])[1])/(self.heatmap_value_maxmin(ChromHeat[heatmapi])[0]-self.heatmap_value_maxmin(ChromHeat[heatmapi])[1]));	
							}
				...
				self.init_HEATMAPsettings();
				////create the HeatMAp objects, pass to d3.Map //d[ChrominitGenome[v.chr]] is the chromsome number generate the values from the chord.groups
				function BioCircosHeatmap(d) {
					return ChromHeat[heatmapi].map(function (v, i) {
						var heatmap_k = (d[ChrominitGenome[v.chr]].endAngle - d[ChrominitGenome[v.chr]].startAngle) / d[ChrominitGenome[v.chr]].value;
						return {
							startAngle: v.start * heatmap_k + d[ChrominitGenome[v.chr]].startAngle,
							endAngle: v.end * heatmap_k + d[ChrominitGenome[v.chr]].startAngle,
							heatmap_chr: v.chr,
							heatmap_start: v.start,
							heatmap_end: v.end,
							heatmap_name: v.name,
			...
			if (self.settings.HEATMAPMouseEvent == true) {
				...
				var HEATMAPMouseOn = svg2.selectAll("path.BioCircosHEATMAP");
	   ...
	   if (self.LINE.length > 0) {
			...
			console.log(self.LINE.length);
			var ChromLine = new Array();
			//for each line object passed to BioCircos
			for (var linei = 0; linei < self.LINE.length; linei++) {
				self.update_LINEsettings(self.LINEConfig[linei]);
				
				var ArrayCeptionLINE = new Array();
				//new reg exp testing for the current chromosome
				var reg = new RegExp(self.genomeLabel[d.index]);
				//add the matching elements in the line array to ArrayCeptionLINE
				for (var r = 0; r < self.LINE[linei].length; r++) {
					if (reg.test(self.LINE[linei][r].chr)) {
						ArrayCeptionLINE.push(self.LINE[linei][r]);
					}
				}
				//push the relevant line value arrays to ChromLine
				ChromLine.push(ArrayCeptionLINE);
			

				//generate line objects
				var line_objects = BioCircosLINE(SingleChord.groups());
				console.log(line_objects);


				
				var line_objects_a_chr = line_objects.filter(function (element, pos) {
					return element.line_chr == self.genomeLabel[d.index]
				})
				//append a g element to the svg and draw appropriate lines in it to display whatever the line module was passed
				svg2.append("g")
					.attr("class", "BioCircosLINE")
				...
				//passes the line object to d3.map and gets the values need to draw lines. 
				function BioCircosLINE(d) {
					return ChromLine[linei].map(function (v, i) { //d[ChrominitGenome[v.chr]] is the chromsome number
						var line_k = (d[ChrominitGenome[v.chr]].endAngle - d[ChrominitGenome[v.chr]].startAngle) / d[ChrominitGenome[v.chr]].value;
						return {
							lin_k: line_k,
							line_angle: v.pos * line_k + d[ChrominitGenome[v.chr]].startAngle,
							line_chr: v.chr,
							line_pos: v.pos,
							line_des: v.des,
							line_value: v.value,
							line_color: "salmon",
							line_width: self.LINEsettings.LineWidth,
							x: (0 + Math.sin(v.pos * line_k + d[ChrominitGenome[v.chr]].startAngle) * ((self.LINEsettings.minRadius + ((v.value - self.line_value_maxmin(ChromLine[linei])[1]) / (self.line_value_maxmin(ChromLine[linei])[0] - self.line_value_maxmin(ChromLine[linei])[1]) * (self.LINEsettings.maxRadius - self.LINEsettings.minRadius))))),
							y: (0 - Math.cos(v.pos * line_k + d[ChrominitGenome[v.chr]].startAngle) * ((self.LINEsettings.minRadius + ((v.value - self.line_value_maxmin(ChromLine[linei])[1]) / (self.line_value_maxmin(ChromLine[linei])[0] - self.line_value_maxmin(ChromLine[linei])[1]) * (self.LINEsettings.maxRadius - self.LINEsettings.minRadius)))))
						};
					});
				}
			...
			if (self.settings.LINEMouseEvent == true) {
				...
				var LINEMouseOn = svg2.selectAll("path.BioCircosLINE");
	...
	//FInal function, BSA draws a BSA if it is passed the number of genomes, preferably only 4 as that is the required amount at this time
    BioCircos.prototype.BSA = function (genome, n) { //and the BioCircos genome object
        ...
        var self = this;
										...
										//mulitply the one genome by 4
										//get it in the right format for display by d3.layout.chord
                                        var GenomeMultiplier = function (genome, n) {
                                            var arr = [];
                                            var zero = [];
                                            var fullArr = [];
                                            var genomeSize = genome.length;
                                            for (var i = 0; i < (genome.length * (n - 1)); i++) {
                                                zero.push(0); //push chromsome numbers * 4 -1 zeros

                                            }

                                            for (var v = 0; v < genome.length; v++) {
                                                for (var z = 0; z < zero.length; z++) {
                                                    genome[v].push(zero[z]);
                                                }
                                            }
                                            ...
                                            for (var i = 0; i < n; i++) {
                                                for (var j = 0; j < genomeSize; j++) {
                                                    arr.push(genome[j]); //add the chromsome length for the chromsomes  at the begining of each array
                                                }
                                            }
                                            return arr;

                                        }
                                        //multiply the genome labels so they match
										var GenomeLableMulitplier = function (genomeLabels, n) {
                                            var Suffix = ["Pa2", "Po2", "Po1", "Pa1"]; // add the suffix to identify which segment the chromosomes are in
                                            var AltLabels = [];
                                            for (var l = 0; l < n; l++) {
                                                for (var labels = 0; labels < genomeLabels.length; labels++) {

                                                    AltLabels.push(genomeLabels[labels] + Suffix[l]);
                                                }
                                            }
                                            return AltLabels;
                                        }										                                    
                                        ...
                                        //names array to label the four quarters
                                        var NamesArray = ["Parent 2 - M+", "Pool 2 - M+", "Pool 1 - WT", "Parent 1 - WT"]
                                        //create a chord variable using the mulitplied genomes
										var chord = d3.layout.chord()
                                                .padding(0.001)
                                                .sortSubgroups(d3.descending)
                                                .matrix(GenomeMultiplier(self.genomeLength, 4));
                                        //svg properties
                                        var width = self.svgWidth,
                                                height = self.svgHeight,
                                                innerRadius = self.innerRadius,
                                        ...
                                        var fill = d3.scale.ordinal()
                                                .domain(d3.range(4))
                                                .range(self.genomeFillColor);
										//background array, is the 90 degrees of the circle  * 4
                                        var fraction = function (n) {
                                            BackArr = [];
                                            for (var i = 0; i < n; i++) {
                                                BackArr[i] = new Array();
                                                for (var j = 0; j < n; j++) {
										...
										//background chord
                                        var BackChord = d3.layout.chord()
                                                .padding(0)
                                                .sortSubgroups(d3.descending)
                                                .matrix(fraction(n));
										//append svg to target div with zoom function and click drag panning
                                        if (self.BSAzoom == true) {
											...
											//append svg to target div without zoom 
                                        } else {
                                            var svg = d3.select(self.target).append("svg")
                                                    .attr("width", width)
                                                    .attr("height", height)
										...
										//creates a fill array for the number of chromsomes in the genome, then clones it so it's four times as long in one array
                                        var FillCalc = function(genome, s){
                                            ...
                                            for(var n = 0; n < s; n++){
                                                ...
                                                newfill.push(fillArray[i]);
                                                }
                                            }
                                            return newfill;
                                            
                                        };										
										//append chromsomes to svg in g element, with matching colours and border
										...
										//draws eye pleasing background
                                        if (self.BSABackground == true) {

                                            svg.append("g").attr("class", "BSABackground")
                                                    .selectAll("path.BSABackground")
                                                    .data(BackChord.groups)
                                                    .enter().append("path")
                                                    .style("fill", function (d) {
                                                        return fill(d.index);
                                                    })
                                                    .attr("d", d3.svg.arc().outerRadius(innerRadius))
                                                    .attr("opacity", 0.5);

                                        }
										//Displays labels for the four quarters
                                        if (self.BSATextDisplay == true) {
                                            var strip = function (value, precision) {
                                                var power = Math.pow(10, precision || 0);
                                                return String(Math.round(value * power) / power);
                                            };
                                        ...
                                        //now for the link module
                                        console.log(self.LINK);
                                        //if there is a link module
										if (self.LINK.length > 0) {
											//create multiplied lables
                                            var MultipliedLabels = GenomeLableMulitplier(self.genomeLabel, n);
                                           
                                            var labeli = MultipliedLabels.length;
                                            var initGenome = new Object();
											//create multiplied labels to number value key pair object
										   for (var labelk = 0; labelk < labeli; labelk++) {
                                                var labelInit = MultipliedLabels[labelk];
                                                initGenome[labelInit] = labelk;
                                            }

                                            console.log(initGenome);
                                            //for all the link objects passed#
											for (var linki = 0; linki < self.LINK.length; linki++) {
                                                self.update_LINKsettings(self.LINKConfig[linki]);
                                                //create link objects
												var link_objects = BioCircosLINK(chord.groups())
                                                console.log(link_objects);
												//append a g element to the svg, place link paths in it genertated using the data in link objects
                                                var Link_svg = svg.append("g")
                                                        .attr("class", "BioCircosLINK")
                                                        .selectAll("path")
                                                        .data(link_objects)
                                                        .enter().append("path")
                                                        .attr("d", function (d) {
                                                            return "M" + d.link_X1 + "," + d.link_Y1 + " Q0,0 " + d.link_X2 + "," + d.link_Y2 + "";
                                                        })
                                                        .attr("class", "BioCircosLINK")
                                                        .attr("fill", "none")
                                                        .attr("stroke", function (d) {
                                                        return d.link_colour;})
                                                ...
                                                function BioCircosLINK(d) {
                                                    return self.LINK[linki].map(function (v, i) {
														//pass to map to split the objects by v
														//get the angles for the start and ends of the links
														//d[initGenome[v.chr]] is the chromsome number it matches the chromsome including the suffix for the correct pool
														//gets number and uses it to direct the link
                                                        ...
                                                        return {
                                                            ...
                                                            link_label2: v.g2name,
                                                            link_pair: v.name,
                                                            link_width: (((v.g1end - v.g1start) * start_k) * innerRadius) * 2,
                                                            link_X1: (0 + Math.sin((v.g1start / 2 + v.g1end / 2) * start_k + d[initGenome[v.g1chr]].startAngle) * (innerRadius)),
                                                            link_Y1: (0 - Math.cos((v.g1start / 2 + v.g1end / 2) * start_k + d[initGenome[v.g1chr]].startAngle) * (innerRadius)),
                                                            link_X2: (0 + Math.sin((v.g2start / 2 + v.g2end / 2) * end_k + d[initGenome[v.g2chr]].startAngle) * (innerRadius)),
                                                            link_Y2: (0 - Math.cos((v.g2start / 2 + v.g2end / 2) * end_k + d[initGenome[v.g2chr]].startAngle) * (innerRadius)),
                                                            link_colour: FillCalc(genome, n)[initGenome[v.g1chr]]           
                                                        };
                                                    });
                                                }
                                            }
											//append divider lines to the svg
                                            if (self.BSADividers == true) {
                                                svg.append("g")
                                                        .attr("class", "BSADividers")
                                                        .append("line")
											...
											//same tooltips as above, div, populated by jquery .html(), we use mouse over and mouse out
                                            if (self.settings.BSALINKMouseEvent == true) {
                                                ...
                                                if (self.settings.BSALINKMouseOverDisplay == true) {
                                                    LINKMouseOn.on("mouseover", function (d) {
                                                        LINKMouseOnTooltip.html(self.settings.LINKMouseOverTooltipsHtml01 + d.link_pair + self.settings.LINKMouseOverTooltipsHtml02) //set tooltip display here
                                                                .style("left", (d3.event.pageX) + "px")
                                                                .style("top", (d3.event.pageY + 20) + "px")
                                                                .style("position", self.settings.LINKMouseOverTooltipsPosition)
                                                                .style("background-color", self.settings.LINKMouseOverTooltipsBackgroundColor)
                                                ...
                                                if (self.settings.BSALINKMouseOutDisplay == true) {
                                                    LINKMouseOn.on("mouseout", function (d) {
                                                        LINKMouseOnTooltip.style("opacity", 0.0);
                                                        d3.select(this)
                                                                .transition()
											...
											//genome mouse event is true also using the mosue over and mouse out function
                                            if (self.settings.BSAGenomeMouseEvent == true) {
                                                var GenomeMouseOnTooltip = d3.select("body")
                                                        .append("div")
                                                        .attr("class", "BioCircosGenomeTooltip")
                                                        .attr("id", "BioCircosGenomeTooltip")
                                                ...
                                                if (self.settings.BSAGenomeMouseOverDisplay == true) {
                                                    var strip = function (value, precision) {
                                                        var power = Math.pow(10, precision || 0);
                                                        return String(Math.round(value * power) / power);
                                                    };
                                                ...
                                                if (self.settings.BSAGenomeMouseOutDisplay == true) {
                                                    GenomeMouseOn.on("mouseout", function (d) {
                                                        GenomeMouseOnTooltip.style("opacity", 0.0);
                                                        d3.select(this)
                                                                .transition()
